...
where $C$ is proportionality constant which depends on the geometry of the discharge tube. Substituting $y'$ value in equation (9.6), we get
...
(a) gravitational force $F_g = mg$  
(b) electric force $F_e = qE$  
(c) buoyant force $F_b$  
(d) viscous force $F_v$
...
Let the gravitational force acting on the oil drop (downward) be $F_g = mg$.
...
where $K = \frac{1}{4\pi\epsilon_0} \frac{2Ze^2}{m v_0^2}$ and $\theta$ is called scattering angle. Equation (9.14) implies that when impact parameter increases, the scattering angle decreases. Smaller the impact parameter, larger will be the deflection of alpha particles.
...
The angular momentum of the electron in these stationary orbits are quantized – that is, it can be written as an integer or integral multiple of $\frac{h}{2\pi}$ called reduced Planck's constant – that is, $\hbar$ (read it as h‑bar) and the integer $n$ is called as principal quantum number.
...
But the de Broglie wavelength $(\lambda)$ associated with an electron of mass $m$ moving with velocity $v$ is $\lambda = \frac{h}{mv}$. Thus from equation (9.15),

$$
2\pi r = n \frac{h}{mv} \quad \Rightarrow \quad m v r = n \frac{h}{2\pi} = n\hbar
$$

For any particle of mass $m$ undergoing circular motion with radius $r$ and velocity $v$, the magnitude of angular momentum $l$ is given by $l = r (m v)$. Thus
...
**EXAMPLE 9.1**  
The radius of the $5^{\mathrm{th}}$ orbit of hydrogen atom is $13.25\ \mathrm{Å}$. Calculate the de Broglie wavelength of the electron orbiting in the $5^{\mathrm{th}}$ orbit.
...
From Bohr's assumption, the angular momentum quantization condition, $m v_n r_n = l_n = n\hbar$

$$
\therefore r_n = \frac{4\pi\epsilon_0 (n\hbar)^2}{Z m e^2} = \frac{\epsilon_0 h^2}{\pi m e^2} \frac{n^2}{Z} \quad (\because \hbar = \frac{h}{2\pi})
$$

Since $\epsilon_0$, $h$, $e$ and $\pi$ are constants, the radius of the orbit becomes

$$
r_n = a_0 \frac{n^2}{Z} \quad \text{where } a_0 = \frac{\epsilon_0 h^2}{\pi m e^2} = 0.529\ \mathrm{Å}
$$

This is known as Bohr radius which is the smallest radius of the orbit in hydrogen atom. Bohr radius is also used as unit of length called Bohr. $1\ \text{Bohr} = 0.53\ \mathrm{Å}$. For hydrogen atom $(Z = 1)$, the radius of $n^{\text{th}}$ orbit is

$$
r_n = a_0 n^2
$$

For $n = 1$ (first orbit or ground state), $r_1 = a_0 = 0.529\ \mathrm{Å}$  
For $n = 2$ (second orbit or first excited state), $r_2 = 4a_0 = 2.116\ \mathrm{Å}$  
For $n = 3$ (third orbit or second excited state), $r_3 = 9a_0 = 4.761\ \mathrm{Å}$  
and so on.
...
Since the electrostatic force is a conservative force, the potential energy for the $n^{\text{th}}$ orbit is
...
Substituting the values of mass and charge of an electron $(m$ and $e)$, permittivity of free space $\epsilon_0$ and Planck's constant $h$ and expressing energy in terms of electron volt $(eV)$, we get

$$
E_n = -13.6 \frac{1}{n^2}\ \mathrm{eV}
$$

For the first orbit (ground state), the total energy of electron is $E_1 = -13.6\ \mathrm{eV}$  
For the second orbit (first excited state), $E_2 = -3.4\ \mathrm{eV}$  
For the third orbit (second excited state), $E_3 = -1.51\ \mathrm{eV}$  
and so on.

Notice that the energy of the first excited state is greater than that of the ground state, second excited state is greater than that of the first excited state and so on. Thus, the orbit which is closest to the nucleus $(r_1)$ has lowest energy (minimum energy compared with other orbits). So, it is often called ground state energy (lowest energy state). The ground state energy of hydrogen $(-13.6\ \mathrm{eV})$ is used as a unit of energy called Rydberg ($1\ \mathrm{Rydberg} = -13.6\ \mathrm{eV}$).
...
**Solution**  
(a) The velocity of an electron in $n^{\text{th}}$ orbit is $v_n = \frac{h}{2\pi m a_0} \frac{Z}{n}$ where $a_0 = \frac{\epsilon_0 h^2}{\pi m e^2} = \text{Bohr radius}$. Substituting for $a_0$ in $v_n$,

$$
v_n = \frac{e^2}{2\epsilon_0 h} \frac{Z}{n} = c \left(\frac{e^2}{2\epsilon_0 h c}\right) \frac{Z}{n} = \frac{\alpha c Z}{n}
$$

where $c$ is the speed of light in free space or vacuum and its value is $c = 3\times 10^8\ \mathrm{ms^{-1}}$ and $\alpha$ is called fine structure constant.
...
For hydrogen atom $(Z = 1)$ the velocity of electron in $n^{\text{th}}$ orbit is

$$
v_n = \frac{c}{137} \frac{1}{n} = (2.19\times 10^6) \frac{1}{n}\ \mathrm{ms^{-1}}
$$

For the first orbit (ground state), $v_1 = 2.19\times 10^6\ \mathrm{ms^{-1}}$  
For the second orbit (first excited state), $v_2 = 1.095\times 10^6\ \mathrm{ms^{-1}}$  
For the third orbit (second excited state), $v_3 = 0.73\times 10^6\ \mathrm{ms^{-1}}$  
Here $v_1 > v_2 > v_3$.
...
Let $\frac{1}{M} + \frac{1}{m} = \frac{1}{\mu_m}$. Here the reduced mass is $\mu_m = \frac{mM}{M+m}$. Therefore, the kinetic energy of the system now is $KE = \frac{p^2}{2\mu_m}$.
...
The excitation energy for an electron from ground state $(n = 1)$ to first excited state $(n = 2)$ is called first excitation energy. For hydrogen atom, it is
...
Excitation potential is defined as excitation energy per unit charge. For hydrogen atom, the first excitation state energy is $E_I = e V_I$  
First excitation potential for hydrogen atom is $V_I = \frac{E_I}{e} = 10.2\ \text{volt}$.  
Similarly, second excitation potential is $V_{II} = \frac{E_{II}}{e} = 12.1\ \text{volt}$ and so on.

#### Ionization energy and ionization potential

An atom is said to be ionized when an electron is completely removed from the atom – that is, it reaches the state with energy $E_{n\to\infty}$. The minimum energy required to remove an electron from an atom in the ground state is known as binding energy or ionization energy. For hydrogen atom, the ground state ionization energy is

$$
E_{\text{ionization}} = E_\infty - E_1 = 0 - (-13.6\ \mathrm{eV}) = 13.6\ \mathrm{eV}
$$

When an electron is in $n^{\text{th}}$ state of an atom, the energy required to remove an electron from that state – that is, the corresponding ionization energy is

$$
E_{\text{ionization}} = E_\infty - E_n = 0 - \left(-13.6 \frac{Z^2}{n^2}\right) = 13.6 \frac{Z^2}{n^2}\ \mathrm{eV}
$$

At normal room temperature, the electron in a hydrogen atom $(Z=1)$ spends most of its time in the ground state. The amount of energy required to remove an electron from the ground state of an atom to the outermost orbit $(E = 0$ for $n \to \infty$) is known as first ionization energy $(13.6\ \mathrm{eV})$. Then, the hydrogen atom is said to be in ionized state or simply called as hydrogen ion, denoted by $H^+$. If we supply more energy than the ionization energy, the excess energy appears as the kinetic energy of the free electron.
...
Thus, for a hydrogen atom $(Z = 1)$, the ionization potential is $V = \frac{13.6}{n^2}\ \text{volt}$.
...
|-------------------|--------------|---------------------|----------------------|
| Radius $(r_n \propto n^2)$ | $0.529\ \mathrm{Å}$ | $2.116\ \mathrm{Å}$ | $4.761\ \mathrm{Å}$ |
| Velocity $(v_n \propto n^{-1})$ | $2.19\times 10^6\ \mathrm{ms^{-1}}$ | $1.095\times 10^6\ \mathrm{ms^{-1}}$ | $0.73\times 10^6\ \mathrm{ms^{-1}}$ |
| Total Energy $(E_n \propto n^{-2})$ | $-13.6\ \mathrm{eV}$ | $-3.4\ \mathrm{eV}$ | $-1.51\ \mathrm{eV}$ |

---

**EXAMPLE 9.5**  
Suppose the energy of an electron in hydrogen‑like atom is given as $E_n = -\frac{54.4}{n^2}\ \mathrm{eV}$ where $n \in \mathbb{N}$. Calculate the following:
...
(c) When a photon with energy $42\ \mathrm{eV}$ and another photon with energy $51\ \mathrm{eV}$ are made to collide with this atom, does this atom absorb these photons?  
...
First excitation energy: $E_2 - E_1 = -13.6 - (-54.4) = 40.8\ \mathrm{eV}$.  
First excitation potential $V_1 = \frac{40.8\ \mathrm{eV}}{e} = 40.8\ \mathrm{V}$.

First ionization energy: $E_\infty - E_1 = 0 - (-54.4) = 54.4\ \mathrm{eV}$.  
Ionization potential $V_{\text{ion}} = \frac{54.4\ \mathrm{eV}}{e} = 54.4\ \mathrm{V}$.

(c) Photon A of $42\ \mathrm{eV}$ and photon B of $51\ \mathrm{eV}$.  
Check differences: $E_2-E_1=40.8\ \mathrm{eV}$, $E_3-E_1=48.36\ \mathrm{eV}$, $E_4-E_1=51\ \mathrm{eV}$.  
So photon B is absorbed (transition $n=1 \to n=4$). Photon A is not absorbed.

(d) Radius of first Bohr orbit: $r_1 = \frac{a_0}{Z} = \frac{0.529}{2} = 0.265\ \mathrm{Å}$.

(e) $KE_n = -E_n$, so for $n=1$, $KE_1 = 54.4\ \mathrm{eV}$. $U_n = -2 KE_n$, so $U_1 = -108.8\ \mathrm{eV}$.
...
Since electrons in excited states have very small life time, these electrons jump back to ground state through spontaneous emission in a short duration of time (approximately $10^{-8}\ \mathrm{s}$) by emitting the radiation with same wavelength (or frequency) corresponding to the colours it absorbed (Figure 9.22 (a)). This is called emission spectroscopy.
...
where $\bar{\nu}$ is known as wave number which is inverse of wavelength, $R$ is known as Rydberg constant whose value is $1.09737\times 10^{7}\ \mathrm{m^{-1}}$ and $m$ and $n$ are positive integers such that $m > n$. The various spectral series are discussed below:
...
Here $A$ is the mass number of the nucleus and the constant $R_0 = 1.2\ \mathrm{F}$, where $1\ \mathrm{F} = 1\times 10^{-15}\ \mathrm{m}$. The unit fermi (F) is named after Enrico Fermi.
...
**Solution**  
From equation (9.19), $R = R_0 A^{1/3}$. Then the volume of the nucleus  
$$
...
It implies that nucleons are extremely tightly packed or compressed state in the nucleus and compare this density with the density of water which is $10^{3}\ \mathrm{kg\ m^{-3}}$.
...
Here $M_A$ is the mass of the atom of an element ${}^A X$.
...
**Solution:**  
Binding energy $BE = \left[ Z m_H + N m_n - M_A \right] c^2$  
...
(1) The value of $\overline{BE}$ rises as the mass number increases until it reaches a maximum value of $8.8\ \mathrm{MeV}$ for $A = 56$ (iron) and then it slowly decreases.  
(2) The average binding energy per nucleon is about $8.5\ \mathrm{MeV}$ for nuclei having mass number lying between $A = 40$ and $120$. These elements are comparatively more stable and not radioactive.  
(3) For higher mass numbers, the curve drops slowly and $\overline{BE}$ for uranium is about $7.6\ \mathrm{MeV}$. Such nuclei are unstable and exhibit radioactivity.  
(4) From Figure 9.24, if two light nuclei with $A<28$ combine to form a nucleus with $A<56$, the binding energy per nucleon is more for the final nucleus than initial nuclei. Thus, if the lighter elements combine to produce a nucleus of medium value $A$, a large amount of energy will be released. This is the basis of nuclear fusion and is the principle of the hydrogen bomb.  
(5) If a nucleus of heavy element is split (fission) into two or more nuclei of medium value $A$, the energy released would again be large. The atom bomb is based on this principle and huge energy of atom bombs comes from this fission when it is uncontrolled. Fission is explained in section 9.7.
...
**Solution**  
From Example 9.9, $BE$ of ${}^{4}_{2}He = 28.33\ \mathrm{MeV}$  
Binding energy per nucleon $= \frac{28.33}{4} \approx 7.08\ \mathrm{MeV}$.
...
Example: Decay of Uranium ${}^{238}_{92}U$ to thorium ${}^{234}_{90}Th$ with the emission of ${}^{4}He$ nucleus ($\alpha$‑particle)

$$
{}^{238}_{92}U \rightarrow {}^{234}_{90}Th + {}^{4}_{2}He
$$

As already mentioned, the total mass of the daughter nucleus and ${}^{4}He$ nucleus is always less than that of the parent nucleus. The difference in mass $(\Delta m = m_X - m_Y - m_\alpha)$ is released as energy called disintegration energy $Q$ and is given by
...
In alpha decay, why does the unstable nucleus emit ${}^{4}He$ nucleus? Why it does not emit four separate nucleons? After all ${}^{4}He$ consists of two protons and two neutrons. For example, if ${}^{238}U$ nucleus decays into ${}^{234}Th$ by emitting four separate nucleons (two protons and two neutrons), then the disintegration energy $Q$ for this process turns out to be negative. It implies that the total mass of products is greater than that of parent $({}^{238}U)$ nucleus. This kind of process cannot occur in nature because it would violate conservation of energy. In any decay process, the conservation of energy, laws of linear momentum and laws of angular momentum must be obeyed.
...
Since $1u = 931\ \mathrm{MeV}$, the energy $Q$ released is  
$$
Q = (0.005812u)\times (931\ \mathrm{MeV}/u) = 5.41\ \mathrm{MeV}
$$

This disintegration energy $Q$ appears as the kinetic energy of $\alpha$ particle and the daughter nucleus. In any decay, the total linear momentum must be conserved. Total linear momentum of the parent nucleus = total linear momentum of the daughter nucleus and $\alpha$ particle.
...
The velocity of $\alpha$ particle $v_\alpha = \frac{m_{Th}}{m_\alpha} v_{Th}$. Since $m_{Th} > m_\alpha$, $v_\alpha > v_{Th}$.
...
The disintegration energy $Q = KE_\alpha + KE_{Th} = 5.41\ \mathrm{MeV}$  
$57 KE_{Th} + KE_{Th} = 5.41\ \mathrm{MeV}$  
$KE_{Th} = \frac{5.41}{58} = 0.093\ \mathrm{MeV}$  
$KE_\alpha = 57 \times 0.093 = 5.301\ \mathrm{MeV}$  
...
(1) it undergoes beta decay directly into ground state carbon $\left({}^{12}_{6}C\right)$ by emitting an electron of maximum energy $13.4\ \mathrm{MeV}$.  
(2) it undergoes beta ray emission to an excited state of carbon $\left({}^{12}_{6}C^*\right)$ by emitting an electron of maximum energy $9.0\ \mathrm{MeV}$ followed by gamma decay to ground state by emitting a photon of energy $4.4\ \mathrm{MeV}$. It is represented by
...
Let us assume that at time $t = 0$ s, the number of nuclei present in the radioactive sample be $N_0$. By integrating the equation (9.31), we can calculate the number of undecayed nuclei $N$ present at any time $t$.
...
where $R_0 = \lambda N_0$. The equation (9.34) is also equivalent to radioactive law of decay. Here $R_0$ is the activity of the sample at $t = 0$ and $R$ is the activity of the sample at any time $t$. From equation (9.34), activity also shows exponential decay behavior. The activity $R$ also can be expressed in terms of number of undecayed atoms present at any time $t$.

From equation (9.34), since $N = N_0 e^{-\lambda t}$ we write

$$
R = \lambda N \tag{9.35}
$$

Equation (9.35) implies that the activity at any time $t$ is equal to the product of decay constant and number of undecayed nuclei present at that time $t$. Since $N$ decreases with time, $R$ also decreases.
...
$1\ \text{Curie} = 1\ \text{Ci} = 3.7\times 10^{10}$ decays per second  
...
The half‑life is the important characteristic of every radioactive sample. Some radioactive nuclei are known to have half‑life as long as $10^{14}$ years and some nuclei have very shorter half‑life time $(10^{-14}\ \mathrm{s})$.
...
where $n$ can be integer or non‑integer. Since the activity of radioactive sample also obeys the exponential decay law, we can also write an equation for activity similar to equation (9.34). After $n$ half‑lives, the activity or decay rate of any radioactive sample is
...
**EXAMPLE 9.13**  
The half‑life of ${}^{238}_{92}U$ against $\alpha$‑decay is $4.5\times 10^9$ years. Calculate the activity of $1\ \mathrm{g}$ sample of ${}^{238}_{92}U$.

**Solution**  
Number of atoms in $1\ \mathrm{g}$ of ${}^{238}_{92}U$:  
$$
N_0 = \frac{6.02\times 10^{23}\ \text{atoms}}{238\ \mathrm{g}} \times 1\ \mathrm{g} = 2.53\times 10^{21}\ \text{atoms}
$$  
Decay constant $\lambda = \frac{0.6931}{T_{1/2}} = \frac{0.6931}{4.5\times 10^9\ \mathrm{yr}} = 1.54\times 10^{-10}\ \mathrm{yr}^{-1}$  
Convert to $\mathrm{s}^{-1}$: $1\ \mathrm{yr} = 3.156\times 10^7\ \mathrm{s}$, so $\lambda = 4.88\times 10^{-18}\ \mathrm{s}^{-1}$  
Activity $R = \lambda N_0 = 4.88\times 10^{-18} \times 2.53\times 10^{21} = 1.23\times 10^{4}\ \mathrm{Bq}$.
...
The radioactive carbon‑14 mixes with the ordinary carbon‑12 and is taken in by plants during photosynthesis and by animals in their food. In a living organism, the ratio of ${}^{14}C$ to ${}^{12}C$ is the same as in the atmosphere. When the organism dies, it stops absorbing $\mathrm{CO}_2$. Since ${}^{14}C$ starts to decay, the ratio of ${}^{14}C$ to ${}^{12}C$ in a dead organism or specimen decreases over the years. Suppose the ratio of ${}^{14}C$ to ${}^{12}C$ in the ancient tree pieces excavated is known, then the age of the tree pieces can be calculated.

---

**EXAMPLE 9.14**  
Keezhadi (Kq), a small hamlet, has become one of the very important archeological places of Tamilnadu. It is located in Sivagangai district. A lot of artefacts (gold coins, pottery, beads, iron tools, jewellery and charcoal, etc.) have been unearthed in Keezhadi which have given substantial evidence that an ancient urban civilization had thrived on the banks of river Vaigai. To determine the age of those materials, the charcoal of $200\ \mathrm{g}$ sent for carbon dating is given in the following figure (b). The activity of ${}^{14}C$ is found to be 37 decays/s. Calculate the age of charcoal.
...
$$  
To find the time $t$ rewriting the above equation (1), $e^{\lambda t} = \frac{R_0}{R}$  
By taking the logarithm on both sides, we get $t = \frac{1}{\lambda} \ln\left(\frac{R_0}{R}\right)$

Here $R = 37\ \mathrm{decays/s} = 37\ \mathrm{Bq}$.  
To find decay constant, $\lambda = \frac{0.6931}{T_{1/2}} = \frac{0.6931}{5730\ \mathrm{yr} \times 3.156\times 10^7\ \mathrm{s/yr}} = 3.83\times 10^{-12}\ \mathrm{s^{-1}}$.

To find the initial activity $R_0$, we use $R_0 = \lambda N_0$. Here $N_0$ is the number of carbon‑14 atoms present in the sample when it was alive. The mass of the charcoal is $200\ \mathrm{g}$. In $12\ \mathrm{g}$ of carbon, there are $6.02\times 10^{23}$ carbon atoms. So $200\ \mathrm{g}$ contains  
$$
\frac{6.02\times 10^{23}\ \mathrm{atoms/mol}}{12\ \mathrm{g/mol}} \times 200 \approx 1\times 10^{25}\ \text{atoms}
$$

When the tree (sample) was alive, the ratio of ${}^{14}C$ to ${}^{12}C$ is $1.3\times 10^{-12}$. So the total number of carbon‑14 atoms is given by  
$$
...
In years, $t = \frac{7.86\times 10^{10}}{3.156\times 10^7} \approx 2500\ \text{years}$.
...
Neutrons are classified according to their kinetic energy as (i) slow neutrons (0 to 1000 eV) (ii) fast neutrons (0.5 MeV to 10 MeV). The neutrons with average energy of about $0.025\ \mathrm{eV}$ in thermal equilibrium are called thermal neutron, because at $298\ \mathrm{K}$, the thermal energy $kT \sim 0.025\ \mathrm{eV}$. Slow and fast neutrons play a vital role in nuclear reactors.
...
We can calculate the energy $(Q)$ released in each uranium fission reaction. We choose the most observed fission reaction which is given in the equation (9.41).

$$
{}^{235}_{92}U + {}^{1}_{0}n \rightarrow {}^{236}_{92}U^* \rightarrow {}^{141}_{56}Ba + {}^{92}_{36}Kr + 3{}^{1}_{0}n + Q
$$

Mass of ${}^{235}_{92}U = 235.045733u$  
Mass of ${}^{1}_{0}n = 1.008665u$  
Total mass of reactants $= 236.054398u$  
Mass of ${}^{141}_{56}Ba = 140.9177u$  
Mass of ${}^{92}_{36}Kr = 91.8854u$  
Mass of $3$ neutrons $= 3.025995u$  
Total mass of products $= 235.829095u$  
Mass defect $\Delta m = 236.054398u - 235.829095u = 0.225303u$  
...
It is possible to calculate the typical energy released in a chain reaction. In the first step, one neutron initiates the fission process in one nucleus by producing three neutrons and energy of about $200\ \mathrm{MeV}$. In the second step, further three nuclei undergo fission, in third step nine nuclei undergo fission, in fourth step 27 nuclei undergo fission and so on. In the $100^{\text{th}}$ step, the number of nuclei which undergo fission is around $2.5\times 10^{40}$. The total energy released after $100^{\text{th}}$ step is $2.5\times 10^{40}\times 200\ \mathrm{MeV} = 8\times 10^{29}\ \mathrm{J}$. It is really an enormous amount of energy which is equivalent to electrical energy required in Tamilnadu for several years.
...
**EXAMPLE 9.15**  
Calculate the amount of energy released when $1\ \mathrm{kg}$ of ${}^{235}_{92}U$ undergoes fission reaction.

**Solution**  
$235\ \mathrm{g}$ of ${}^{235}_{92}U$ has $6.02\times 10^{23}$ atoms. In one gram of ${}^{235}_{92}U$, the number of atoms is equal to $\frac{6.02\times 10^{23}}{235} = 2.56\times 10^{21}$.  
So the number of atoms present in $1\ \mathrm{kg}$ of ${}^{235}_{92}U = 2.56\times 10^{21} \times 1000 = 2.56\times 10^{24}$.  
Each ${}^{235}_{92}U$ nucleus releases $200\ \mathrm{MeV}$ of energy during the fission. The total energy released by $1\ \mathrm{kg}$ of ${}^{235}_{92}U$ is  
$$
...
$$  
This is enormously large amount of energy which is enough to keep $100\ \mathrm{W}$ bulb operating for 30,000 years. To produce this much energy by chemical reaction, around 20,000 tons of TNT (tri nitro toluene) has to be exploded.
...
**Shielding:** For a protection against harmful radiations, the nuclear reactor is surrounded by a concrete wall of thickness of about $2$ to $2.5\ \mathrm{m}$.
...
To overcome this repulsion, the two light nuclei must have enough kinetic energy to move closer to each other such that the nuclear force becomes effective. This can be achieved if the temperature is very much greater than $10^7\ \mathrm{K}$. When the surrounding temperature reaches around $10^7\ \mathrm{K}$, lighter nuclei start fusing to form heavier nuclei and this resulting reaction is called thermonuclear fusion reaction.

### Energy generation in stars

The natural place where nuclear fusion occurs is the core of the stars, since their temperature is of the order of $10^7\ \mathrm{K}$. In fact, the energy generation in every star is only through thermonuclear fusion. In most of the stars including our Sun hydrogen atoms fuse into helium and in some stars helium atoms fuse into heavier elements.
...
The overall energy produced in the above reactions is about $27\ \mathrm{MeV}$. The radiation energy we receive from the sun is due to these fusion reactions.
...
- The impact parameter $(b)$ is defined as the perpendicular distance between the centre of the gold nucleus and the direction of velocity vector of alpha particle when it is at a large distance.
...
- The radius of first orbit hydrogen atom is $a_0 = \frac{\epsilon_0 h^2}{\pi m e^2} = 0.529\ \mathrm{Å}$ and it is also known as Bohr radius.
- The velocity of electron in $n^{\text{th}}$ orbit is $v_n = \frac{h}{2\pi m a_0} \frac{Z}{n}\ \mathrm{ms^{-1}}$.
- The fine structure constant is $\alpha = \frac{1}{137}$ which is a dimensionless constant.
- The total energy of electron in the $n^{\text{th}}$ orbit is $E_n = -\frac{m e^4}{8\epsilon_0^2 h^2} \frac{Z^2}{n^2} = -13.6 \frac{Z^2}{n^2}\ \mathrm{eV}$.
...
- The nucleus of element $X$ having atomic number $Z$ and mass number $A$ is represented by ${}^A X$.
- The empirical relation connecting radius of nucleus $(Z > 10)$ $R$ and mass number $A$ is given by $R = R_0 A^{1/3}$ where $R_0 = 1.2\ \mathrm{F}$.
- The density of nucleus $\rho = 2.3\times 10^{17}\ \mathrm{kg\ m^{-3}}$, and the nuclear matter is in a highly compressed state.
- If $M$, $m_p$ and $m_n$ are masses of a nucleus $({}^A X)$, proton and neutron respectively, then the mass defect is $\Delta m = \left(Z m_p + N m_n\right) - M$.
- The binding energy of nucleus $BE = \left(Z m_p + N m_n - M\right) c^2$.
- The average binding energy per nucleon is maximum for iron which is $8.8\ \mathrm{MeV}$.
- Alpha decay: ${}_Z^A X \rightarrow {}_{Z-2}^{A-4} Y + {}_2^4 He$.
- $\beta^-$ decay: ${}_Z^A X \rightarrow {}_{Z+1}^A Y + e^- + \bar{\nu}$.
- $\beta^+$ decay: ${}_Z^A X \rightarrow {}_{Z-1}^A Y + e^+ + \nu$.
- Gamma emission: ${}_Z^A X^* \rightarrow {}_Z^A X + \gamma$.
...
- Mean life $\tau = \frac{1}{\lambda}$, $T_{1/2} = 0.6931\,\tau$.
...
1. Suppose an alpha particle accelerated by a potential of $V$ volt is allowed to collide with a nucleus of atomic number $Z$, then the distance of closest approach of alpha particle to the nucleus is  
   (a) $14.4 \frac{Z}{V}\ \mathrm{Å}$ (b) $14.4 \frac{V}{Z}\ \mathrm{Å}$ (c) $1.44 \frac{Z}{V}\ \mathrm{Å}$ (d) $1.44 \frac{V}{Z}\ \mathrm{Å}$
...
6. In J.J. Thomson $e/m$ experiment, electrons are accelerated through $2.6\ \mathrm{kV}$ enter the region of crossed electric field and magnetic field of strength $3.0\times 10^4\ \mathrm{Vm^{-1}}$ and $1.0\times 10^{-3}\ \mathrm{T}$ respectively, and pass through it and undeflected, then the specific charge is  
   (a) $1.6\times 10^{10}\ \mathrm{Ckg^{-1}}$ (b) $1.7\times 10^{11}\ \mathrm{Ckg^{-1}}$ (c) $1.5\times 10^{11}\ \mathrm{Ckg^{-1}}$ (d) $1.8\times 10^{11}\ \mathrm{Ckg^{-1}}$
...
8. The electric potential of an electron is given by $V = V_0 \ln\left(\frac{r}{r_0}\right)$, where $r_0$ is a constant. If Bohr atom model is valid, then variation of radius of $n^{\text{th}}$ orbit $r_n$ with the principal quantum number $n$ is  
   (a) $r_n \propto \frac{1}{n}$ (b) $r_n \propto n$ (c) $r_n \propto \frac{1}{n^2}$ (d) $r_n \propto n^2$

9. If the nuclear radius of ${}^{27}Al$ is $3.6\ \mathrm{fermi}$, the approximate nuclear radius of ${}^{64}Cu$ in fermi is  
...
11. The mass of a ${}^{7}Li$ nucleus is $0.042\ u$ less than the sum of the masses of all its nucleons. The average binding energy per nucleon of ${}^{7}Li$ nucleus is nearly  
    (a) $46\ \mathrm{MeV}$ (b) $5.6\ \mathrm{MeV}$ (c) $3.9\ \mathrm{MeV}$ (d) $23\ \mathrm{MeV}$

12. $M_p$ denotes the mass of the proton and $M_n$ denotes mass of a neutron. A given nucleus of binding energy $B$, contains $Z$ protons and $N$ neutrons. The mass $M(N,Z)$ of the nucleus is given by (where $c$ is the speed of light)  
    (a) $M(N,Z) = N M_n + Z M_p - B c^2$ (b) $M(N,Z) = N M_n + Z M_p + B c^2$ (c) $M(N,Z) = N M_n + Z M_p - B / c^2$ (d) $M(N,Z) = N M_n + Z M_p + B / c^2$
...
14. The half‑life period of a radioactive element $A$ is same as the mean life time of another radioactive element $B$. Initially both have the same number of atoms. Then  
    (a) $A$ and $B$ have the same decay rate initially (b) $A$ and $B$ decay at the same rate always (c) $B$ will decay at faster rate than $A$ (d) $A$ will decay at faster rate than $B$

15. A radioactive element has $N_0$ number of nuclei at $t = 0$. The number of nuclei remaining after half of a half‑life (that is, at time $t = \frac{1}{2} T_{1/2}$)  
    (a) $\frac{N_0}{2}$ (b) $\frac{N_0}{\sqrt{2}}$ (c) $\frac{N_0}{4}$ (d) $\frac{N_0}{8}$
...
22. In alpha decay, why the unstable nucleus emits ${}^{4}_{2}He$ nucleus? Why it does not emit four separate nucleons?  
...
1. Consider two hydrogen atoms $H_A$ and $H_B$ in ground state. Assume that hydrogen atom $H_A$ is at rest and hydrogen atom $H_B$ is moving with a speed and makes head‑on collision with the stationary hydrogen atom $H_A$. After the collision, both of them move together. What is the minimum value of the kinetic energy of the moving hydrogen atom $H_B$, such that any one of the hydrogen atoms reaches first excitation state.  
   [Ans: $20.4\ \mathrm{eV}$]

2. In the Bohr atom model, the frequency of transitions is given by the following expression  
   $\nu = Rc\left(\frac{1}{n^2} - \frac{1}{m^2}\right)$, where $n < m$.  
   Consider the following transitions:  
   $m\to n$: $3\to 2$, $2\to 1$, $3\to 1$  
   Show that the frequency of these transitions obey sum rule (which is known as Ritz combination principle)  
   [Ans: $\nu_{3\to 2} + \nu_{2\to 1} = \nu_{3\to 1}$]

3. (a) A hydrogen atom is excited by radiation of wavelength $97.5\ \mathrm{nm}$. Find the principal quantum number of the excited state.  
...
5. Calculate the mass defect and the binding energy per nucleon of the ${}^{108}_{47}Ag$ nucleus. (atomic mass of $\mathrm{Ag} = 107.905949\ u$)  
   [Ans: $\Delta m = 0.990391\ u$, $B.E = 8.5\ \mathrm{MeV}$]
...
7. On your birthday, you measure the activity of the sample ${}^{210}Bi$ which has a half‑life of $5.01$ days. The initial activity that you measure is $1\ \mu\mathrm{Ci}$.  
   ...
   [Ans: (a) $10^{-22}\ \mu\mathrm{Ci}$, (b) $1.6\times 10^{-6}\ \mathrm{s^{-1}}$, (c) $7.23\ \mathrm{days}$, (d) $2.31\times 10^{10}$]

8. Calculate the time required for $60\%$ of a sample of radon to undergo decay. Given $T_{1/2}$ of radon $= 3.8$ days.  
   [Ans: $5.022$ days]

9. Assuming that energy released by the fission of a single ${}^{235}_{92}U$ nucleus is $200\ \mathrm{MeV}$, calculate the number of fissions per second required to produce $1$ watt power.  
   [Ans: $3.125\times 10^{10}$]

10. Show that the mass of radium $\left({}^{226}_{88}Ra\right)$ with an activity of $1$ curie is almost a gram. Given $T_{1/2} = 1600$ years.
...
2. Concepts of Modern Physics, Arthur Beiser, McGraw Hill, $6^{\text{th}}$ edition  
...
5. Physics for scientist and engineers with modern physics, Serway and Jewett, Brook/Cooler publishers, $8^{\text{th}}$ edition  
...
- Click 'START' button of stop watch and notice the time taken $t_1$ by a drop, to travel distance $l_1$ between any two points. Calculate the terminal velocity $v_1 = \frac{l_1}{t_1}$.  
- Click 'Voltage On' to suspend the same oil drop in air, which is the balancing voltage $V$.  
- Click the 'X Ray ON' button and notice the time taken $t_2$ by same drop to travel distance $l_2$ between any two points. Calculate the terminal velocity $v_2 = \frac{l_2}{t_2}$.  
