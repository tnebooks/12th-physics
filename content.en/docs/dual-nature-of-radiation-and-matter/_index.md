...
The minimum energy needed for an electron to escape from the metal surface is called work function of that metal. The work function of the metal is denoted by {{< katex display=true >}}\phi_0{{< /katex >}} and is measured in electron volt (eV).
...
1\ \mathrm{eV} = KE\ \text{gained by the electron} = \text{Work done by the electric field} = qV = 1.602\times 10^{-19}\ \mathrm{C} \times 1\ \mathrm{V} = 1.602\times 10^{-19}\ \mathrm{J}
{{< katex display=true >}}{{< /katex >}}

Suppose the maximum kinetic energy of the free electron inside the metal is {{< katex display=true >}}0.5\ \mathrm{eV}$ and the energy needed to overcome the surface barrier of a metal is $3\ \mathrm{eV}$ then the minimum energy needed for electron emission from the metallic surface is $3 - 0.5 = 2.5\ \mathrm{eV}$. Here $2.5\ \mathrm{eV}{{< /katex >}} is the work function of the metal.
...
In 1902, Lenard studied this electron emission phenomenon in detail. His simple experimental setup is shown in Figure 8.7. The apparatus consists of two metallic plates {{< katex display=true >}}A$ and $C$ placed in an evacuated quartz bulb. The galvanometer $G$ and battery $B{{< /katex >}} are connected in the circuit.

![Figure 8.7 Experimental setup of Lenard](image-placeholder)

When ultraviolet light is incident on the negative plate {{< katex display=true >}}C{{< /katex >}} an electric current flows in the circuit that is indicated by the deflection in the galvanometer. On other hand, if the positive plate is irradiated by the ultraviolet light, no current is observed in the circuit.

From these observations, it is concluded that when ultraviolet light falls on the negative plate, electrons are ejected from it which are attracted by the positive plate {{< katex display=true >}}A{{< /katex >}}. On reaching the positive plate through the evacuated bulb, the circuit is completed and the current flows in it. Thus, the ultraviolet light falling on the negative plate causes the electron emission from the surface of the plate.
...
The apparatus shown in Figure 8.8 is employed to study the phenomenon of photoelectric effect in detail. {{< katex display=true >}}S$ is a source of electromagnetic waves of known and variable frequency $\nu$ and intensity $I$. $C$ is the cathode (negative electrode) made up of photosensitive material and is used to emit electrons. The anode (positive electrode) $A$ collects the electrons emitted from $C{{< /katex >}}. These electrodes are kept in an evacuated glass envelope with a quartz window that permits the passage of ultraviolet and visible light.

The necessary potential difference between {{< katex display=true >}}C$ and $A$ is provided by high tension battery $B$ which is connected across a potential divider arrangement $PQ$ through a key $K$. $C$ is connected to the centre terminal while $A$ to the sliding contact $J$ of the potential divider. The plate $A$ can be maintained at a desired positive or negative potential with respect to $C$. To measure both positive and negative potential of $A$ with respect to $C$, the voltmeter is designed to have its zero marking at the centre and is connected between $A$ and $C$. The current is measured by a micro ammeter $\mu A{{< /katex >}} connected in series.

If there is no light falling on the cathode {{< katex display=true >}}C$, no photoelectrons are emitted and the microammeter reads zero. When ultraviolet or visible light is allowed to fall on $C{{< /katex >}}, the photoelectrons are liberated and are attracted towards anode. As a result, the photoelectric current is set up in the circuit which is measured using micro ammeter.
...
To study the effect of intensity of incident light on photoelectric current, the frequency of the incident light and the accelerating potential {{< katex display=true >}}V$ of the anode are kept constant. Here the potential of $A$ is kept positive with respect to that of $C$ so that the electrons emitted from $C$ are attracted towards $A{{< /katex >}}. Now, the intensity of the incident light is varied and the corresponding photoelectric current is measured.

![Figure 8.9 Variation of photocurrent with intensity](image-placeholder)

A graph is drawn between light intensity along {{< katex display=true >}}x$‑axis and the photocurrent along $y{{< /katex >}}‑axis. From the graph in Figure 8.9, it is evident that photocurrent – the number of electrons emitted per second – is directly proportional to the intensity of the incident light.

### 8.2.3 Effect of potential difference on photoelectric current

To study the effect of potential difference {{< katex display=true >}}V$ between the electrodes on photoelectric current, the frequency and intensity of the incident light are kept constant. Initially the potential of $A$ is kept positive with respect to $C{{< /katex >}} and the cathode is irradiated with the given light.

Now, the potential of {{< katex display=true >}}A$ is increased and the corresponding photocurrent is noted. As the potential of $A$ is increased, photocurrent also increases. However a stage is reached where photocurrent reaches a saturation value (saturation current) at which all the photoelectrons from $C$ are collected by $A$. This is represented by the flat portion of the graph between potential of $A{{< /katex >}} and photocurrent (Figure 8.10).

When a negative (retarding) potential is applied to {{< katex display=true >}}A$ with respect to $C$, the current does not immediately drop to zero because the photoelectrons are emitted with some definite and different kinetic energies. The kinetic energy of some of the photoelectrons is such that they could overcome the retarding electric field and reach the electrode $A{{< /katex >}}.

When the negative (retarding) potential of {{< katex display=true >}}A$ is gradually increased, the photocurrent starts to decrease because more and more photoelectrons are being repelled away from reaching the electrode $A$. The photocurrent becomes zero at a particular negative potential $V_0{{< /katex >}}, called stopping or cut‑off potential.

![Figure 8.10 Variation of photocurrent with potential difference](image-placeholder)

Stopping potential is that value of the negative (retarding) potential given to the collecting electrode {{< katex display=true >}}A{{< /katex >}} which is just sufficient to stop the most energetic photoelectrons emitted and make the photocurrent zero.

At the stopping potential, even the most energetic electron is brought to rest. Therefore, the initial kinetic energy of the fastest electron {{< katex display=true >}}(K_{\max})$ is equal to the work done by the stopping potential to stop it $(eV_0){{< /katex >}}.

{{< katex display=true >}}{{< /katex >}}
K_{\max} = \frac{1}{2} m v_{\max}^{2} = eV_0 \tag{8.1}
{{< katex display=true >}}{{< /katex >}}

where {{< katex display=true >}}v_{\max}{{< /katex >}} is the maximum speed of the emitted photoelectron.

{{< katex display=true >}}{{< /katex >}}
v_{\max} = \sqrt{\frac{2eV_0}{m}} = \sqrt{\frac{2\times 1.602\times 10^{-19}}{9.1\times 10^{-31}}} \times \sqrt{V_0} = 5.93\times 10^{5} \sqrt{V_0}
{{< katex display=true >}}{{< /katex >}}

From equation (8.1),

{{< katex display=true >}}{{< /katex >}}
K_{\max} = eV_0\ \text{(in joule)} \qquad \text{or} \qquad K_{\max} = V_0\ \text{(in eV)} \tag{8.3, 8.4}
{{< katex display=true >}}{{< /katex >}}

From the Figure 8.10, when the intensity of the incident light alone is increased, the saturation current also increases but the value of {{< katex display=true >}}V_0{{< /katex >}} remains constant.
...
iii) Since the energy of light is spread across the entire wavefront, the electrons which receive energy from it are large in number. Each electron needs considerable amount of time (a few hours) to get energy sufficient to overcome the work function and to get liberated from the surface. But experiments show that photoelectric emission is almost instantaneous process (the time lag is less than {{< katex display=true >}}10^{-9}{{< /katex >}} s after the surface is illuminated) which could not be explained by wave theory.
...
i) maximum kinetic energy of the photoelectrons {{< katex display=true >}}(K_{\max})$ depends on the intensity $I{{< /katex >}} of the incident light  
ii) {{< katex display=true >}}K_{\max}{{< /katex >}} does not depend on the frequency of the incident light and  
...
b) For a given element, each atom absorbs an equal amount of energy and this energy is proportional to its cross-sectional area {{< katex display=true >}}A{{< /katex >}}  
c) Each atom gives this energy to one of the electrons.

(Given : The work function for cesium is {{< katex display=true >}}2.14\ \mathrm{eV}$ and the power absorbed per unit area is $1.60\times 10^{-6}\ \mathrm{Wm^{-2}}{{< /katex >}} which produces a measurable photocurrent in cesium.)

**Solution**  
i) According to wave theory, the energy in a light wave is spread out uniformly and continuously over the wavefront.  
The energy absorbed by each electron in time {{< katex display=true >}}t{{< /katex >}} is given by  
{{< katex display=true >}}{{< /katex >}}
E = I A t
{{< katex display=true >}}{{< /katex >}}  
With this energy absorbed, the most energetic electron is released with {{< katex display=true >}}K_{\max}$ by overcoming the surface energy barrier or work function $\phi_0{{< /katex >}} and this is expressed as  
{{< katex display=true >}}{{< /katex >}}
K_{\max} = I A t - \phi_0 \tag{1}
{{< katex display=true >}}{{< /katex >}}  
Thus, wave theory predicts that for a unit time, at low light intensities when {{< katex display=true >}}IA < \phi_0$, no electrons are emitted. At higher intensities, when $IA \ge \phi_0$, electrons are emitted. This implies that higher the light intensity, greater will be $K_{\max}{{< /katex >}}.  
{{< katex display=true >}}K_{\max}{{< /katex >}} is dependent only on the intensity under given conditions – that is, by suitably increasing the intensity, one can produce photoelectric effect even if the frequency is less than the threshold frequency. So the concept of threshold frequency does not even exist in wave theory.

ii) According to wave theory, the intensity of a light wave is proportional to the square of the amplitude of the electric field {{< katex display=true >}}(E_0^2){{< /katex >}}. The amplitude of this electric field increases with increasing intensity and imparts an increasing acceleration and kinetic energy to an electron.  
Now {{< katex display=true >}}I$ is replaced with a quantity proportional to $E_0^2$ in equation (1). This means that $K_{\max}{{< /katex >}} should not depend at all on the frequency of the classical light wave which again contradicts the experimental results.
...
{{< katex display=true >}}{{< /katex >}}
0 = I A t - \phi_0 \quad \Rightarrow \quad t = \frac{\phi_0}{I A} = \frac{\phi_0}{I (\pi r^2)}
{{< katex display=true >}}{{< /katex >}}  
By taking the atomic radius {{< katex display=true >}}r = 1.0\times 10^{-10}\ \mathrm{m}$ and substituting the given values of $I$ and $\phi_0{{< /katex >}}, we can estimate the time interval as  
{{< katex display=true >}}{{< /katex >}}
t = \frac{2.14\times 1.6\times 10^{-19}}{1.60\times 10^{-6}\times 3.14\times (1\times 10^{-10})^2} = 0.68\times 10^{7}\ \mathrm{s} \approx 79\ \text{days}
{{< katex display=true >}}{{< /katex >}}  
...
i) If an oscillator vibrates with frequency {{< katex display=true >}}\nu{{< /katex >}} its energy can have only certain discrete values, given by the equation

{{< katex display=true >}}{{< /katex >}}
E_n = n h \nu \qquad n = 1,2,3,\dots \tag{8.5}
{{< katex display=true >}}{{< /katex >}}

where {{< katex display=true >}}h{{< /katex >}} is a constant, called Planck's constant.

ii) The oscillators emit or absorb energy in small packets or quanta and the energy of each quantum is {{< katex display=true >}}h\nu{{< /katex >}}.
...
Einstein extended Planck's quantum concept to explain the photoelectric effect in 1905. According to Einstein, the energy in light is not spread out over wavefronts but is concentrated in small packets or energy quanta. Therefore, light (or any other electromagnetic waves) of frequency {{< katex display=true >}}\nu$ from any source can be considered as a stream of quanta and the energy of each light quantum is given by $E = h\nu{{< /katex >}}.

He also proposed that a quantum of light has linear momentum and the magnitude of that linear momentum is {{< katex display=true >}}p = \frac{h\nu}{c}{{< /katex >}}. The individual light quantum of definite energy and momentum can be associated with a particle. The light quantum can behave as a particle and this is called photon. Therefore, photon is nothing but particle manifestation of light.
...
i) The photons of light of frequency {{< katex display=true >}}\nu$ and wavelength $\lambda{{< /katex >}} will have energy, given by
{{< katex display=true >}}{{< /katex >}}
E = h\nu = \frac{hc}{\lambda}.
{{< katex display=true >}}{{< /katex >}}
...
According to quantum concept, intensity of light of given wavelength is defined as the number of energy quanta or photons incident per unit area per unit time, with each photon having same energy. Its unit is {{< katex display=true >}}\mathrm{Wm^{-2}}{{< /katex >}}.

#### Einstein's explanation of photoelectric equation

When a photon of energy {{< katex display=true >}}h\nu$ is incident on a metal surface, it is completely absorbed by a single electron and the electron is ejected. In this process, a part of the photon energy is used in overcoming the potential barrier of the metal surface (photoelectric work function $\phi_0{{< /katex >}}) and the remaining energy as the kinetic energy of the ejected electron. From the law of conservation of energy,

{{< katex display=true >}}{{< /katex >}}
h\nu = \phi_0 + \frac{1}{2} m v^2 \tag{8.6}
{{< katex display=true >}}{{< /katex >}}

where {{< katex display=true >}}m$ is the mass of the electron and $v{{< /katex >}} its velocity. This is shown in Figure 8.13(a).

![Figure 8.13 Emission of photoelectrons](image-placeholder)

If we reduce the frequency of the incident light, the speed or kinetic energy of photo electrons is also reduced. At some frequency {{< katex display=true >}}\nu_0{{< /katex >}} of incident radiation, the photo electrons are just ejected with almost zero kinetic energy (Figure 8.13(b)). Then the equation (8.6) becomes

{{< katex display=true >}}{{< /katex >}}
h\nu_0 = \phi_0
{{< katex display=true >}}{{< /katex >}}

where {{< katex display=true >}}\nu_0{{< /katex >}} is the threshold frequency. By rewriting the equation (8.6), we get

{{< katex display=true >}}{{< /katex >}}
h\nu = h\nu_0 + \frac{1}{2} m v^2 \tag{8.7}
{{< katex display=true >}}{{< /katex >}}

The equation (8.7) is known as Einstein's photoelectric equation.

If the electron does not lose energy by internal collisions, then it is emitted with maximum kinetic energy {{< katex display=true >}}K_{\max}{{< /katex >}}. Then

{{< katex display=true >}}{{< /katex >}}
K_{\max} = \frac{1}{2} m v_{\max}^2
{{< katex display=true >}}{{< /katex >}}

where {{< katex display=true >}}v_{\max}{{< /katex >}} is the maximum velocity of the electron ejected. The equation (8.6) is rearranged as follows:

{{< katex display=true >}}{{< /katex >}}
K_{\max} = h\nu - \phi_0 \tag{8.8}
{{< katex display=true >}}{{< /katex >}}

![Figure 8.14 {{< katex display=true >}}K_{\max}$ vs $\nu{{< /katex >}} graph](image-placeholder)

A graph between maximum kinetic energy {{< katex display=true >}}K_{\max}$ of the photoelectron and frequency $\nu$ of the incident light is a straight line as shown in Figure 8.14. The slope of the line is $h$ and its $y$‑intercept is $-\phi_0{{< /katex >}}.

Einstein's equation was experimentally verified by R.A. Millikan. He drew {{< katex display=true >}}K_{\max}$ versus $\nu{{< /katex >}} graph for many metals (cesium, potassium, sodium and lithium) as shown in Figure 8.15 and found that the slope is independent of the metals.

![Figure 8.15 {{< katex display=true >}}K_{\max}$ vs $\nu{{< /katex >}} graph for different metals](image-placeholder)

Millikan also calculated the value of Planck's constant {{< katex display=true >}}(h = 6.626\times 10^{-34}\ \mathrm{Js}){{< /katex >}} and work function of many metals (Cs, K, Na, Ca); these values are in agreement with the theoretical prediction.
...
ii) From {{< katex display=true >}}K_{\max} = h\nu - \phi_0$ it is evident that $K_{\max}{{< /katex >}} is proportional to the frequency of the incident light and is independent of intensity of the light.
...
The cathode {{< katex display=true >}}C$ is semi‑cylindrical in shape and is coated with a photo sensitive material. The anode $A$ is a thin rod or wire kept along the axis of the semi‑cylindrical cathode. A potential difference is applied between the anode and the cathode through a galvanometer $G{{< /katex >}}.
...
**EXAMPLE 8.2**  
A radiation of wavelength {{< katex display=true >}}300\ \mathrm{nm}$ is incident on a silver surface. Will photoelectrons be observed? [work function of silver $= 4.7\ \mathrm{eV}{{< /katex >}}]

**Solution:**  
Energy of the incident photon is  
{{< katex display=true >}}{{< /katex >}}
E = h\nu = \frac{hc}{\lambda}\ \text{(in joules)} = \frac{hc}{\lambda e}\ \text{(in eV)}
{{< katex display=true >}}{{< /katex >}}  
Substituting the known values, we get  
{{< katex display=true >}}{{< /katex >}}
E = \frac{6.626\times 10^{-34}\times 3\times 10^{8}}{300\times 10^{-9}\times 1.6\times 10^{-19}} = 4.14\ \mathrm{eV}
{{< katex display=true >}}{{< /katex >}}  
The work function of silver {{< katex display=true >}}= 4.7\ \mathrm{eV}{{< /katex >}}. Since the energy of the incident photon is less than the work function of silver, photoelectrons are not observed in this case.

---

**EXAMPLE 8.3**  
When light of wavelength {{< katex display=true >}}2200\ \mathrm{Å}$ falls on Cu, photo electrons are emitted from it. Find (i) the threshold wavelength and (ii) the stopping potential. Given: the work function for Cu is $\phi_0 = 4.65\ \mathrm{eV}{{< /katex >}}.

**Solution**  
(i) The threshold wavelength is given by  
{{< katex display=true >}}{{< /katex >}}
\lambda_0 = \frac{hc}{\phi_0} = \frac{6.626\times 10^{-34}\times 3\times 10^{8}}{4.65\times 1.6\times 10^{-19}} = 2672\ \mathrm{Å}
{{< katex display=true >}}{{< /katex >}}

(ii) Energy of the photon of wavelength {{< katex display=true >}}2200\ \mathrm{Å}{{< /katex >}} is  
{{< katex display=true >}}{{< /katex >}}
E = \frac{hc}{\lambda} = \frac{6.626\times 10^{-34}\times 3\times 10^{8}}{2200\times 10^{-10}} = 9.035\times 10^{-19}\ \mathrm{J} = 5.65\ \mathrm{eV}
{{< katex display=true >}}{{< /katex >}}  
We know that kinetic energy of fastest photo electron is  
{{< katex display=true >}}{{< /katex >}}
K_{\max} = h\nu - \phi_0 = 5.65 - 4.65 = 1\ \mathrm{eV}
{{< katex display=true >}}{{< /katex >}}  
From equation (8.3), {{< katex display=true >}}K_{\max} = eV_0{{< /katex >}}  
{{< katex display=true >}}{{< /katex >}}
V_0 = \frac{K_{\max}}{e} = \frac{1\times 1.6\times 10^{-19}}{1.6\times 10^{-19}} = 1\ \mathrm{V}
{{< katex display=true >}}{{< /katex >}}  
Therefore, stopping potential {{< katex display=true >}}= 1\ \mathrm{V}{{< /katex >}}.

---

**EXAMPLE 8.4**  
The work function of potassium is {{< katex display=true >}}2.30\ \mathrm{eV}$. UV light of wavelength $3000\ \mathrm{Å}$ and intensity $2\ \mathrm{Wm^{-2}}{{< /katex >}} is incident on the potassium surface.  
i) Determine the maximum kinetic energy of the photo electrons  
ii) If {{< katex display=true >}}40\%$ of incident photons produce photo electrons, how many electrons are emitted per second if the area of the potassium surface is $2\ \mathrm{cm^2}{{< /katex >}}?

**Solution**  
i) The energy of the incident photon is  
{{< katex display=true >}}{{< /katex >}}
E = \frac{hc}{\lambda} = \frac{6.626\times 10^{-34}\times 3\times 10^{8}}{3000\times 10^{-10}} = 6.626\times 10^{-19}\ \mathrm{J} = 4.14\ \mathrm{eV}
{{< katex display=true >}}{{< /katex >}}  
Maximum KE of the photoelectrons is  
{{< katex display=true >}}{{< /katex >}}
K_{\max} = h\nu - \phi_0 = 4.14 - 2.30 = 1.84\ \mathrm{eV}
{{< katex display=true >}}{{< /katex >}}

ii) The number of photons reaching the surface per second is  
{{< katex display=true >}}{{< /katex >}}
n_p = \frac{I}{E} \times A = \frac{2}{6.626\times 10^{-19}} \times 2\times 10^{-4} = 6.04\times 10^{14}\ \text{photons/sec}
{{< katex display=true >}}{{< /katex >}}  
...
**EXAMPLE 8.5**  
Light of wavelength {{< katex display=true >}}390\ \mathrm{nm}$ is directed at a metal electrode. To find the energy of electrons ejected, an opposing potential difference is established between it and another electrode. The current of photoelectrons from one to the other is stopped completely when the potential difference is $1.10\ \mathrm{V}{{< /katex >}}. Determine  
i) the work function of the metal and  
ii) the maximum wavelength of light that can eject electrons from this metal.
...
{{< katex display=true >}}{{< /katex >}}  
(since {{< katex display=true >}}K_{\max} = eV_0{{< /katex >}})  
{{< katex display=true >}}{{< /katex >}}
= \left[\frac{6.626\times 10^{-34}\times 3\times 10^{8}}{390\times 10^{-9}}\right] - \left[1.6\times 10^{-19}\times 1.10\right]
{{< katex display=true >}}{{< /katex >}}  
{{< katex display=true >}}{{< /katex >}}
= 5.10\times 10^{-19} - 1.76\times 10^{-19} = 3.34\times 10^{-19}\ \mathrm{J} = 2.09\ \mathrm{eV}
{{< katex display=true >}}{{< /katex >}}

ii) The threshold wavelength is  
{{< katex display=true >}}{{< /katex >}}
\lambda_0 = \frac{hc}{\phi_0} = \frac{6.626\times 10^{-34}\times 3\times 10^{8}}{3.34\times 10^{-19}} = 5.951\times 10^{-7}\ \mathrm{m} = 5951\ \mathrm{Å}
{{< katex display=true >}}{{< /katex >}}
...
The momentum of photon of frequency {{< katex display=true >}}\nu{{< /katex >}} is given by  
{{< katex display=true >}}{{< /katex >}}
p = \frac{h\nu}{c} = \frac{h}{\lambda} \qquad \text{since } c = \nu\lambda
{{< katex display=true >}}{{< /katex >}}  
The wavelength of a photon in terms of its momentum is  
{{< katex display=true >}}{{< /katex >}}
\lambda = \frac{h}{p} \tag{8.9}
{{< katex display=true >}}{{< /katex >}}  
According to de Broglie, the above equation is completely a general one and this is applicable to material particles as well. Therefore, for a particle of mass {{< katex display=true >}}m$ travelling with speed $v{{< /katex >}}, the wavelength is given by  
{{< katex display=true >}}{{< /katex >}}
\lambda = \frac{h}{mv} = \frac{h}{p} \tag{8.10}
{{< katex display=true >}}{{< /katex >}}  
This wavelength of the matter waves is known as de Broglie wavelength. This equation relates the wave character (the wavelength {{< katex display=true >}}\lambda$) and the particle character (the momentum $p{{< /katex >}}) through Planck's constant.

### 8.3.3 De Broglie wavelength of electrons

Let an electron of mass {{< katex display=true >}}m$ be accelerated through a potential difference of $V{{< /katex >}} volt. The kinetic energy acquired by the electron is given by  
{{< katex display=true >}}{{< /katex >}}
\frac{1}{2} m v^2 = eV
{{< katex display=true >}}{{< /katex >}}  
Therefore, the speed {{< katex display=true >}}v{{< /katex >}} of the electron is  
{{< katex display=true >}}{{< /katex >}}
v = \sqrt{\frac{2eV}{m}} \tag{8.11}
{{< katex display=true >}}{{< /katex >}}  
Hence, the de Broglie wavelength of the matter waves associated with electron is  
{{< katex display=true >}}{{< /katex >}}
\lambda = \frac{h}{mv} = \frac{h}{\sqrt{2emV}}
{{< katex display=true >}}{{< /katex >}}  
Substituting the known values in the above equation, we get  
{{< katex display=true >}}{{< /katex >}}
\lambda = \frac{6.626\times 10^{-34}}{\sqrt{2V\times 1.6\times 10^{-19}\times 9.11\times 10^{-31}}} = \frac{12.27\times 10^{-10}}{\sqrt{V}}\ \mathrm{m} \quad \text{or} \quad \lambda = \frac{12.27}{\sqrt{V}}\ \mathrm{Å} \tag{8.12}
{{< katex display=true >}}{{< /katex >}}  
For example, if an electron is accelerated through a potential difference of {{< katex display=true >}}100\ \mathrm{V}$ then its de Broglie wavelength is $1.227\ \mathrm{Å}{{< /katex >}}.

Since the kinetic energy of the electron, {{< katex display=true >}}K = eV{{< /katex >}}, then the de Broglie wavelength associated with electron can be also written as  
{{< katex display=true >}}{{< /katex >}}
...
The filament {{< katex display=true >}}F{{< /katex >}} is heated by a low tension (L.T.) battery. Electrons are emitted from the hot filament by thermionic emission. They are then accelerated due to the potential difference between the filament and the anode aluminium cylinder by a high tension (H.T.) battery. Electron beam is collimated by using two thin aluminium diaphragms and is allowed to strike a single crystal of Nickel.

![Figure 8.17 Experimental set up of Davisson – Germer experiment](image-placeholder)

The electrons scattered by Ni atoms in different directions are received by the electron detector which measures the intensity of scattered electron beam. The detector is capable of rotation in the plane of the paper so that the angle {{< katex display=true >}}\theta$ between the incident beam and the scattered beam can be changed at our will. The intensity of the scattered electron beam is measured as a function of the angle $\theta{{< /katex >}}.

![Figure 8.18 Variation of intensity of diffracted electron beam with the angle {{< katex display=true >}}\theta{{< /katex >}}](image-placeholder)
...
We have learnt in unit 7 that as the wavelength of light is very small, diffraction effects of light are very small. In order to study diffraction of light, diffraction gratings are used. Since x‑rays and de Broglie waves of electrons have wavelengths (in the order of {{< katex display=true >}}10^{-10}\ \mathrm{m}{{< /katex >}}) much shorter than that of the light wave, diffraction grating cannot be used in x‑ray diffraction studies. In a crystal, the spacing between atomic planes is comparable to the wavelength of x‑rays and de Broglie waves of electrons. Hence, in x‑ray diffraction studies, the crystals are used which serve as three‑dimensional grating.

Figure 8.18 shows the variation of intensity of the scattered electrons with the angle {{< katex display=true >}}\theta$ for the accelerating voltage of $54\ \mathrm{V}$. For a given accelerating voltage $V$, the scattered wave shows a peak or maximum at an angle of $50^\circ$ to the incident electron beam. This peak in intensity is attributed to the constructive interference of electrons diffracted from various atomic layers of the target material. From the known value of interplanar spacing of Nickel, the wavelength of the electron wave was experimentally calculated as $1.65\ \mathrm{Å}{{< /katex >}}.

The wavelength can also be calculated from de Broglie relation for {{< katex display=true >}}V = 54\ \mathrm{V}{{< /katex >}} from equation (8.12).  
{{< katex display=true >}}{{< /katex >}}
\lambda = \frac{12.27}{\sqrt{54}}\ \mathrm{Å} = 1.67\ \mathrm{Å}
{{< katex display=true >}}{{< /katex >}}  
This value agrees very well with the experimentally observed wavelength of {{< katex display=true >}}1.65\ \mathrm{Å}{{< /katex >}}. Thus this experiment directly verifies de Broglie's hypothesis of the wave nature of moving particles.
...
i) an electron with kinetic energy {{< katex display=true >}}2\ \mathrm{eV}{{< /katex >}}  
ii) a bullet of {{< katex display=true >}}50\ \mathrm{g}$ fired from rifle with a speed of $200\ \mathrm{m/s}{{< /katex >}}  
iii) a {{< katex display=true >}}4000\ \mathrm{kg}$ car moving along the highways at $50\ \mathrm{m/s}{{< /katex >}}  
...
{{< katex display=true >}}{{< /katex >}}
p = \sqrt{2mK} = \sqrt{2\times 9.1\times 10^{-31}\times 2\times 1.6\times 10^{-19}} = 7.63\times 10^{-25}\ \mathrm{kg\ m\ s^{-1}}
{{< katex display=true >}}{{< /katex >}}  
Its de Broglie wavelength is  
{{< katex display=true >}}{{< /katex >}}
\lambda = \frac{h}{p} = \frac{6.626\times 10^{-34}}{7.63\times 10^{-25}} = 0.868\times 10^{-9}\ \mathrm{m} = 8.68\ \mathrm{Å}
{{< katex display=true >}}{{< /katex >}}

ii) Momentum of the bullet is  
{{< katex display=true >}}{{< /katex >}}
p = mv = 0.050\times 200 = 10\ \mathrm{kg\ m\ s^{-1}}
{{< katex display=true >}}{{< /katex >}}  
Its de Broglie wavelength is  
{{< katex display=true >}}{{< /katex >}}
...
{{< katex display=true >}}{{< /katex >}}
p = mv = 4000\times 50 = 2\times 10^{5}\ \mathrm{kg\ m\ s^{-1}}
{{< katex display=true >}}{{< /katex >}}  
Its de Broglie wavelength is  
{{< katex display=true >}}{{< /katex >}}
...
From these calculations, we notice that electron has significant value of de Broglie wavelength {{< katex display=true >}}(\approx 10^{-9}\ \mathrm{m}$ which can be measured from diffraction studies) but moving bullet and car have negligibly small de Broglie wavelengths associated with them $(\approx 10^{-33}\ \mathrm{m}$ and $10^{-39}\ \mathrm{m}{{< /katex >}} respectively, which are not measurable by any experiment). This implies that the wave nature of matter is important at the atomic level but it is not really relevant at the macroscopic level.

---

**EXAMPLE 8.7**  
Find the de Broglie wavelength associated with an alpha particle which is accelerated through a potential difference of {{< katex display=true >}}400\ \mathrm{V}$. Given that the mass of the proton is $1.67\times 10^{-27}\ \mathrm{kg}{{< /katex >}}.

**Solution**  
An alpha particle contains 2 protons and 2 neutrons. Therefore, the mass {{< katex display=true >}}M$ of the alpha particle is 4 times that of a proton $(m_p)$ (or a neutron) and its charge $q$ is twice that of a proton $(+2e){{< /katex >}}.

The de Broglie wavelength associated with it is  
{{< katex display=true >}}{{< /katex >}}
\lambda = \frac{h}{\sqrt{2MqV}} = \frac{h}{\sqrt{2\times (4m_p)\times (2e)\times V}}
{{< katex display=true >}}{{< /katex >}}  
{{< katex display=true >}}{{< /katex >}}
= \frac{6.626\times 10^{-34}}{\sqrt{2\times 4\times 1.67\times 10^{-27}\times 2\times 1.6\times 10^{-19}\times 400}}
{{< katex display=true >}}{{< /katex >}}  
{{< katex display=true >}}{{< /katex >}}
= \frac{6.626\times 10^{-34}}{4\times 20\times 10^{-23}\sqrt{1.67\times 1.6}} = 0.00507\ \mathrm{Å}
{{< katex display=true >}}{{< /katex >}}
...
**Solution**  
We know that {{< katex display=true >}}\lambda = \frac{h}{\sqrt{2mK}}{{< /katex >}}. Since proton and electron have same de Broglie wavelength, we get  
{{< katex display=true >}}{{< /katex >}}
\frac{h}{\sqrt{2m_p K_p}} = \frac{h}{\sqrt{2m_e K_e}} \quad \Rightarrow \quad \frac{K_p}{K_e} = \frac{m_e}{m_p}
{{< katex display=true >}}{{< /katex >}}  
Since {{< katex display=true >}}m_e < m_p$, $K_p < K_e{{< /katex >}}; the electron has more kinetic energy than the proton.

Also {{< katex display=true >}}\lambda = \frac{h}{mv}$ gives $v = \frac{h}{m\lambda}$. For same $\lambda$, $v \propto \frac{1}{m}$. Since $m_e < m_p$, $v_e > v_p{{< /katex >}}; the electron moves faster than the proton.
...
Quantum theory of radiation explains photoelectric effect in which the electrons are emitted due to the incidence of photons and the energy is transferred from photons to the electrons. Immediately, a question arises: Is the reverse process also possible? This means that whether kinetic energy of electron can be transformed into photon energy or not. The phenomenon which answers this question has already been discovered, even before Planck's quantum theory of radiation.
...
X‑rays are produced in x‑ray tube which is essentially a discharge tube as shown in Figure 8.20. A tungsten filament {{< katex display=true >}}F{{< /katex >}} is heated to incandescence by a battery. As a result, electrons are emitted from it by thermionic emission.

The electrons are accelerated to high speeds by the voltage applied between the filament {{< katex display=true >}}F$ and the anode. The target materials like tungsten, molybdenum are embedded in the face of the solid copper anode. The face of the target is inclined at an angle of $45^\circ{{< /katex >}} with respect to the electron beam so that x‑rays can leave the tube through its side.
...
The continuous spectrum consists of radiations of all possible wavelengths with a certain minimum wavelength {{< katex display=true >}}\lambda_0{{< /katex >}} which depends on the voltage across the electrodes. The peaks are characteristics of the material of the target and hence it is called characteristic spectrum. Figure 8.21(a) depicts the x‑ray spectra of tungsten at various accelerating voltages and Figure 8.21(b) shows the x‑ray spectra of tungsten and molybdenum at a particular accelerating voltage.

![Figure 8.21 (a) X‑ray spectra of tungsten at various accelerating potentials](image-placeholder)
![Figure 8.21 (b) X‑ray spectra of tungsten and molybdenum at {{< katex display=true >}}35\ \mathrm{kV}{{< /katex >}} accelerating potential](image-placeholder)
...
When an electron gives up all its energy, then the photon is emitted with highest frequency {{< katex display=true >}}\nu_0$ or lowest wavelength $\lambda_0$. The initial kinetic energy of an electron is given by $eV$ where $V{{< /katex >}} is the accelerating voltage. Therefore, we have  
{{< katex display=true >}}{{< /katex >}}
h\nu_0 = eV \quad \text{or} \quad \frac{hc}{\lambda_0} = eV
{{< katex display=true >}}{{< /katex >}}  
{{< katex display=true >}}{{< /katex >}}
\lambda_0 = \frac{hc}{eV}
{{< katex display=true >}}{{< /katex >}}  
where {{< katex display=true >}}\lambda_0{{< /katex >}} is the cut‑off wavelength. Substituting the known values in the above equation, we get  
{{< katex display=true >}}{{< /katex >}}
\lambda_0 = \frac{12400}{V}\ \mathrm{Å} \tag{8.14}
{{< katex display=true >}}{{< /katex >}}  
The relation given by equation (8.14) is known as the Duane‑Hunt formula.

The value of {{< katex display=true >}}\lambda_0{{< /katex >}} depends only on the accelerating potential and is same for all targets. This is in good agreement with the experimental results. Thus, the production of continuous x‑ray spectrum and the origin of cut‑off wavelength can be explained on the basis of photon theory of radiation.
...
**EXAMPLE 8.9**  
Calculate the cut‑off wavelength and cut‑off frequency of x‑rays from an x‑ray tube of accelerating potential {{< katex display=true >}}20,000\ \mathrm{V}{{< /katex >}}.

**Solution**  
The cut‑off wavelength of the x‑rays in the continuous spectrum is given by  
{{< katex display=true >}}{{< /katex >}}
\lambda_0 = \frac{12400}{V}\ \mathrm{Å} = \frac{12400}{20000}\ \mathrm{Å} = 0.62\ \mathrm{Å}
{{< katex display=true >}}{{< /katex >}}  
The corresponding frequency is  
{{< katex display=true >}}{{< /katex >}}
\nu_0 = \frac{c}{\lambda_0} = \frac{3\times 10^{8}}{0.62\times 10^{-10}} = 4.84\times 10^{18}\ \mathrm{Hz}
{{< katex display=true >}}{{< /katex >}}
...
- {{< katex display=true >}}1\ \mathrm{eV}$ is equal to $1.602\times 10^{-19}\ \mathrm{J}{{< /katex >}}.
...
- According to Planck, matter is composed of a large number of oscillating particles (atoms) which vibrate with different frequencies.
...
- Continuous x‑ray spectrum consists of radiations of all possible wavelengths with a certain minimum wavelength {{< katex display=true >}}\lambda_0{{< /katex >}}.
...
1. The wavelength {{< katex display=true >}}\lambda_{\mathrm{e}}$ of an electron and $\lambda_{\mathrm{p}}$ of a photon of same energy $E{{< /katex >}} are related by (NEET 2013)  
   a) {{< katex display=true >}}\lambda_{\mathrm{p}} \propto \lambda_{\mathrm{e}}$ b) $\lambda_{\mathrm{p}} \propto \sqrt{\lambda_{\mathrm{e}}}$ c) $\lambda_{\mathrm{p}} \propto \frac{1}{\sqrt{\lambda_{\mathrm{e}}}}$ d) $\lambda_{\mathrm{p}} \propto \lambda_{\mathrm{e}}^{2}{{< /katex >}}

2. In an electron microscope, the electrons are accelerated by a voltage of {{< katex display=true >}}14\ \mathrm{kV}$. If the voltage is changed to $224\ \mathrm{kV}{{< /katex >}} then the de Broglie wavelength associated with the electrons would  
   a) increase by 2 times b) decrease by 2 times c) decrease by 4 times d) increase by 4 times

3. The wave associated with a moving particle of mass {{< katex display=true >}}3\times 10^{-6}\ \mathrm{g}$ has the same wavelength as an electron moving with a velocity $6\times 10^{6}\ \mathrm{m\ s^{-1}}{{< /katex >}}. The velocity of the particle is  
   a) {{< katex display=true >}}1.82\times 10^{-18}\ \mathrm{m\ s^{-1}}$ b) $9\times 10^{-2}\ \mathrm{m\ s^{-1}}$ c) $3\times 10^{-31}\ \mathrm{m\ s^{-1}}$ d) $1.82\times 10^{-15}\ \mathrm{m\ s^{-1}}{{< /katex >}}

4. When a metallic surface is illuminated with radiation of wavelength {{< katex display=true >}}\lambda$ the stopping potential is $V$. If the same surface is illuminated with radiation of wavelength $2\lambda$ the stopping potential is $\frac{V}{4}{{< /katex >}}. The threshold wavelength for the metallic surface is (NEET 2016)  
   a) {{< katex display=true >}}4\lambda$ b) $5\lambda$ c) $\frac{5}{2}\lambda$ d) $3\lambda{{< /katex >}}

5. If a light of wavelength {{< katex display=true >}}330\ \mathrm{nm}$ is incident on a metal with work function $3.55\ \mathrm{eV}$ the electrons are emitted. Then the wavelength of the wave associated with the emitted electron is (Take $h = 6.6\times 10^{-34}\ \mathrm{Js}{{< /katex >}})  
   a) {{< katex display=true >}}< 2.75\times 10^{-9}\ \mathrm{m}$ b) $\ge 2.75\times 10^{-9}\ \mathrm{m}$ c) $\le 2.75\times 10^{-12}\ \mathrm{m}$ d) $< 2.5\times 10^{-10}\ \mathrm{m}{{< /katex >}}

6. A photoelectric surface is illuminated successively by monochromatic light of wavelength {{< katex display=true >}}\lambda$ and $\frac{\lambda}{2}{{< /katex >}}. If the maximum kinetic energy of the emitted photoelectrons in the second case is 3 times that in the first case, the work function of the material is (NEET 2015)  
   a) {{< katex display=true >}}\frac{hc}{\lambda}$ b) $\frac{2hc}{\lambda}$ c) $\frac{hc}{3\lambda}$ d) $\frac{hc}{2\lambda}{{< /katex >}}
...
7. In photoelectric emission, a radiation whose frequency is 4 times threshold frequency of a certain metal is incident on the metal. Then the maximum possible velocity of the emitted electron will be  
   a) {{< katex display=true >}}\sqrt{\frac{h\nu_0}{m}}$ b) $\sqrt{\frac{6h\nu_0}{m}}$ c) $2\sqrt{\frac{h\nu_0}{m}}$ d) $\sqrt{\frac{h\nu_0}{2m}}{{< /katex >}}

8. Two radiations with photon energies {{< katex display=true >}}0.9\ \mathrm{eV}$ and $3.3\ \mathrm{eV}$ respectively are falling on a metallic surface successively. If the work function of the metal is $0.6\ \mathrm{eV}{{< /katex >}} then the ratio of maximum speeds of emitted electrons in the two cases will be  
   a) 1:4 b) 1:3 c) 1:1 d) 1:9

9. A light source of wavelength {{< katex display=true >}}520\ \mathrm{nm}$ emits $1.04\times 10^{15}$ photons per second while the second source of $460\ \mathrm{nm}$ produces $1.38\times 10^{15}{{< /katex >}} photons per second. Then the ratio of power of second source to that of first source is  
   a) 1.00 b) 1.02 c) 1.5 d) 0.98

10. If the mean wavelength of light from sun is taken as {{< katex display=true >}}550\ \mathrm{nm}$ and its mean power as $3.8\times 10^{26}\ \mathrm{W}{{< /katex >}}, then the number of photons emitted per second from the sun is of the order of  
    a) {{< katex display=true >}}10^{45}$ b) $10^{42}$ c) $10^{54}$ d) $10^{51}{{< /katex >}}

11. The threshold wavelength for a metal surface whose photoelectric work function is {{< katex display=true >}}3.313\ \mathrm{eV}{{< /katex >}} is  
    a) {{< katex display=true >}}4125\ \mathrm{Å}$ b) $3750\ \mathrm{Å}$ c) $6000\ \mathrm{Å}$ d) $2062.5\ \mathrm{Å}{{< /katex >}}

12. A light of wavelength {{< katex display=true >}}500\ \mathrm{nm}$ is incident on a sensitive metal plate of photoelectric work function $1.235\ \mathrm{eV}$. The kinetic energy of the photo electrons emitted is (Take $h = 6.6\times 10^{-34}\ \mathrm{Js}{{< /katex >}})  
    a) {{< katex display=true >}}0.58\ \mathrm{eV}$ b) $2.48\ \mathrm{eV}$ c) $1.24\ \mathrm{eV}$ d) $1.16\ \mathrm{eV}{{< /katex >}}

13. Photons of wavelength {{< katex display=true >}}\lambda$ are incident on a metal. The most energetic electrons ejected from the metal are bent into a circular arc of radius $R$ by a perpendicular magnetic field having magnitude $B{{< /katex >}}. The work function of the metal is (KVPY‑SX 2016)  
    a) {{< katex display=true >}}\frac{hc}{\lambda} - m_e + \frac{e^2 B^2 R^2}{2m_e}$ b) $\frac{hc}{\lambda} + 2m_e\left[\frac{eBR}{2m_e}\right]^2$ c) $\frac{hc}{\lambda} - m_e c^2 - \frac{e^2 B^2 R^2}{2m_e}$ d) $\frac{hc}{\lambda} - 2m_e\left[\frac{eBR}{2m_e}\right]^2{{< /katex >}}

14. The work functions for metals {{< katex display=true >}}A$, $B$ and $C$ are $1.92\ \mathrm{eV}$, $2.0\ \mathrm{eV}$ and $5.0\ \mathrm{eV}$ respectively. The metal/metals which will emit photoelectrons for a radiation of wavelength $4100\ \mathrm{Å}{{< /katex >}} is/are  
    a) A only b) both {{< katex display=true >}}A$ and $B{{< /katex >}} c) all these metals d) none
...
8. Write the expression for the de Broglie wavelength associated with a charged particle of charge {{< katex display=true >}}q$ and mass $m$, when it is accelerated through a potential $V{{< /katex >}}.  
...
12. Write the relationship of de Broglie wavelength {{< katex display=true >}}\lambda$ associated with a particle of mass $m$ in terms of its kinetic energy $K{{< /katex >}}.  
...
15. Give the applications of photocell.  
16. How do we obtain characteristic x‑ray spectra?

### IV. Numerical problems

1. How many photons per second emanate from a {{< katex display=true >}}50\ \mathrm{mW}$ laser of $640\ \mathrm{nm}{{< /katex >}}?  
   [Ans: {{< katex display=true >}}1.61\times 10^{17}\ \mathrm{s^{-1}}{{< /katex >}}]

2. Calculate the maximum kinetic energy and maximum velocity of the photoelectrons emitted when the stopping potential is {{< katex display=true >}}81\ \mathrm{V}{{< /katex >}} for the photoelectric emission experiment.  
   [Ans: {{< katex display=true >}}1.3\times 10^{-17}\ \mathrm{J}$, $5.3\times 10^{6}\ \mathrm{ms^{-1}}{{< /katex >}}]

3. Calculate the energies of the photons associated with the following radiation:  
   (i) violet light of {{< katex display=true >}}413\ \mathrm{nm}$ (ii) X‑rays of $0.1\ \mathrm{nm}$ (iii) radio waves of $10\ \mathrm{m}{{< /katex >}}  
   [Ans: {{< katex display=true >}}3\ \mathrm{eV}$, $12424\ \mathrm{eV}$, $1.24\times 10^{-7}\ \mathrm{eV}{{< /katex >}}]

4. A {{< katex display=true >}}150\ \mathrm{W}$ lamp emits light of mean wavelength of $5500\ \mathrm{Å}$. If the efficiency is $12\%{{< /katex >}}, find out the number of photons emitted by the lamp in one second.  
   [Ans: {{< katex display=true >}}4.98\times 10^{19}{{< /katex >}}]

5. How many photons of frequency {{< katex display=true >}}10^{14}\ \mathrm{Hz}$ will make up $19.86\ \mathrm{J}{{< /katex >}} of energy?  
   [Ans: {{< katex display=true >}}3\times 10^{20}{{< /katex >}}]

6. What should be the velocity of the electron so that its momentum equals that of {{< katex display=true >}}4000\ \mathrm{Å}{{< /katex >}} wavelength photon.  
   [Ans: {{< katex display=true >}}1818\ \mathrm{ms^{-1}}{{< /katex >}}]

7. When a light of frequency {{< katex display=true >}}9\times 10^{14}\ \mathrm{Hz}$ is incident on a metal surface, photoelectrons are emitted with a maximum speed of $8\times 10^{5}\ \mathrm{ms^{-1}}{{< /katex >}}. Determine the threshold frequency of the surface.  
   [Ans: {{< katex display=true >}}4.61\times 10^{14}\ \mathrm{Hz}{{< /katex >}}]

8. When a {{< katex display=true >}}6000\ \mathrm{Å}$ light falls on the cathode of a photo cell, photoemission takes place. If a potential of $0.8\ \mathrm{V}{{< /katex >}} is required to stop emission of electron, then determine the  
   (i) frequency of the light (ii) energy of the incident photon (iii) work function of the cathode material (iv) threshold frequency (v) net energy of the electron after it leaves the surface.  
   [Ans: {{< katex display=true >}}5\times 10^{14}\ \mathrm{Hz}$, $2.07\ \mathrm{eV}$, $1.27\ \mathrm{eV}$, $3.07\times 10^{14}\ \mathrm{Hz}$, $0.8\ \mathrm{eV}{{< /katex >}}]

9. A {{< katex display=true >}}3310\ \mathrm{Å}$ photon liberates an electron from a material with energy $3\times 10^{-19}\ \mathrm{J}$ while another $5000\ \mathrm{Å}$ photon ejects an electron with energy $0.972\times 10^{-19}\ \mathrm{J}{{< /katex >}} from the same material. Determine the value of Planck's constant and the threshold wavelength of the material.  
   [Ans: {{< katex display=true >}}6.62\times 10^{-34}\ \mathrm{Js}$, $6620\times 10^{-10}\ \mathrm{m}{{< /katex >}}]

10. At the given point of time, the earth receives energy from sun at {{< katex display=true >}}4\ \mathrm{cal\ cm^{-2}\ min^{-1}}$. Determine the number of photons received on the surface of the Earth per $\mathrm{cm^2}$ per minute. (Given: Mean wavelength of sun light $= 5500\ \mathrm{Å}{{< /katex >}})  
    [Ans: {{< katex display=true >}}4.65\times 10^{19}{{< /katex >}}]

11. UV light of wavelength {{< katex display=true >}}1800\ \mathrm{Å}$ is incident on a lithium surface whose threshold wavelength is $4965\ \mathrm{Å}{{< /katex >}}. Determine the maximum energy of the electron emitted.  
    [Ans: {{< katex display=true >}}4.40\ \mathrm{eV}{{< /katex >}}]

12. Calculate the de Broglie wavelength of a proton whose kinetic energy is equal to {{< katex display=true >}}81.9\times 10^{-15}\ \mathrm{J}{{< /katex >}}. (Given: mass of proton is 1836 times that of electron).  
    [Ans: {{< katex display=true >}}4\times 10^{-14}\ \mathrm{m}{{< /katex >}}]

13. A deuteron and an alpha particle are accelerated with the same potential. Which one of the two has  
    i) greater value of de Broglie wavelength associated with it and  
    ii) less kinetic energy? Explain.  
    [Ans: {{< katex display=true >}}\lambda_d = 2\lambda_\alpha$ and $K_d = \frac{K_\alpha}{2}{{< /katex >}}]

14. An electron is accelerated through a potential difference of {{< katex display=true >}}81\ \mathrm{V}{{< /katex >}}. What is the de Broglie wavelength associated with it? To which part of electromagnetic spectrum does this wavelength correspond?  
    [Ans: {{< katex display=true >}}\lambda = 1.36\ \mathrm{Å}{{< /katex >}} and x‑rays]

15. The ratio between the de Broglie wavelength associated with proton, accelerated through a potential of {{< katex display=true >}}512\ \mathrm{V}$ and that of alpha particle accelerated through a potential of $X$ volts is found to be one. Find the value of $X{{< /katex >}}.  
    [Ans: {{< katex display=true >}}64\ \mathrm{V}{{< /katex >}}]
...
3. H.C.Verma, *Concepts of Physics*, Volume 1 and 2, Bharathi Bhavan publishers.  
