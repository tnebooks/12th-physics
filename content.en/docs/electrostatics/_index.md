...
> [Image: Figure 1.1 (a) Unlike charges attract each other (b) Like charges repel each other]
...
> [Image: Figure 1.2 Coulomb force between two positive point charges]
...
(ii) The force on the charge $q_{2}$ exerted by the charge $q_{1}$ always lies along the line joining the two charges. $\hat{r}_{12}$ is the unit vector pointing from charge $q_{1}$ to $q_{2}$ . It is shown in the Figure 1.2. Likewise, the force on the charge $q_{1}$ exerted by $q_{2}$ is along $- \hat{r}_{12}$ (i.e., in the direction opposite to $\hat{r}_{12}$ ).
...
> [Image: Diagram of two hanging charged spheres]
...
> [Image: Free body diagram of a charged sphere]
...
$$\quad = 2\times 0.1\times \sin 30^{\circ}\times \sqrt{\frac{10^{-3}\times 10\times\tan 30^{\circ}}{9\times 10^{9}}}$$
...
$$\quad = \frac{9\times 2.56}{28.09}\times 10^{-7} = 8.2\times 10^{-8}\mathrm{N}$$
...
$$\qquad = \frac{6.67\times 10^{-11}\times 9.1\times 10^{-31}\times 1.6\times 10^{-27}}{\left(5.3\times 10^{-11}\right)^{2}}$$
$$\qquad = \frac{97.11}{28.09}\times 10^{-47} = 3.4\times 10^{-47}\mathrm{N}$$
...
> [Image: Figure 1.3 Electrostatic attraction between a comb and pieces of papers]
...
$$\begin{array}{rl} & {\bar{F}_{1}^{tot} = \bar{F}_{12} + \bar{F}_{13} + \bar{F}_{14} + \dots \dots \dots \bar{F}_{1n}}\\ & {\bar{F}_{1}^{tot} = k\left\{\frac{q_{1}q_{2}}{r_{21}^{2}}\hat{r}_{21} + \frac{q_{1}q_{3}}{r_{31}^{2}}\hat{r}_{31} + \frac{q_{1}q_{4}}{r_{41}^{2}}\hat{r}_{41} + \dots \right.}\\ & {\qquad \left.\dots +\frac{q_{1}q_{n}}{r_{n1}^{2}}\hat{r}_{n1}\right\} \qquad (1.3)} \end{array}$$

> [Image: Diagram for superposition principle]
...
> [Image: Figure showing four charges on a circle]
...
> [Image: Diagram showing force vectors on q1]
...
$$\begin{array}{rl} & {\bar{F}_{12} = F_{12}\cos \theta \hat{i} -F_{12}\sin \theta \hat{j}}\\ & {\quad = 4.5\times 10^{-3}\times \frac{1}{\sqrt{2}}\hat{i} -4.5\times 10^{-3}\times \frac{1}{\sqrt{2}}\widehat{j}}\\ & {\bar{F}_{13} = F_{13}\hat{i} = 2.25\times 10^{-3}\mathrm{N}\hat{i}}\\ & {\bar{F}_{14} = F_{14}\cos \theta \hat{i} +F_{14}\sin \theta \hat{j}}\\ & {\quad = 4.5\times 10^{-3}\times \frac{1}{\sqrt{2}}\hat{i} +4.5\times 10^{-3}\times \frac{1}{\sqrt{2}}\widehat{j}} \end{array}$$

Then the total force on $q_{1}$ is

$$\bar{F}_{1}^{tot} = \left(F_{12}\cos \theta \hat{i} -F_{12}\sin \theta \hat{j}\right) + F_{13}\hat{i}$$
$$\qquad +\left(F_{14}\cos \theta \hat{i} +F_{14}\sin \theta \hat{j}\right)$$
$$\bar{F}_{1}^{tot} = \left(F_{12}\cos \theta +F_{13} + F_{14}\cos \theta\right)\hat{i}$$
$$\qquad +\left(-F_{12}\sin \theta +F_{14}\sin \theta\right)\hat{j}$$
...
$$\begin{array}{rl} & {\quad = \left(\frac{4.5}{\sqrt{2}} +2.25 + \frac{4.5}{\sqrt{2}}\right)\times 10^{-3}\hat{i}}\\ & {\quad = \left(4.5\sqrt{2} +2.25\right)\times 10^{-3}\hat{i}}\\ & {\bar{F}_{1}^{tot} = 8.61\times 10^{-3}\hat{i}\mathrm{~N}} \end{array}$$
...
$$\bar{F} = \frac{kq q_{0}}{r^{2}}\hat{r} = \frac{1}{4\pi\epsilon_{0}}\frac{q q_{0}}{r^{2}}\hat{r}\mathrm{~where~}k = \frac{1}{4\pi\epsilon_{0}}$$
...
> [Image: Figure 1.4 Electric field of positive and negative charges]
...
> [Image: Figure 1.5 Coulomb's law in terms of electric field]
...
Note that in Figures 1.6 (a) and (b) the length of the electric field vector is shown for three different points. The strength or magnitude of the electric field at point $\mathrm{P}$ is stronger than at the points $\mathrm{Q}$ and $\mathrm{R}$ because the point $\mathrm{P}$ is closer to the source charge.

> [Image: Figure 1.6 (a) Electric field due to positive charge (b) Electric field due to negative charge]
...
> [Image: Figure 1.7 Uniform and non-uniform electric field]

(vii) The expression (1.4) is valid only for point charges. For continuous and finite size charge distributions, integration techniques must be used (Refer Appendix A1.1). However, this expression can be used as an approximation for a finite- sized charge if the test point is very far away from the finite sized source charge. Note that we similarly treat the Earth as a point mass when we calculate the gravitational field of the Sun on the Earth (Refer unit 6, volume 2, XI physics).
...
> [Image: Diagram showing points P and Q at distances from origin]
...
> [Image: Figure 1.8 Superposition of Electric field]
...
> [Image: Diagram showing charges and point A]
...
$$\vec{E}_A = \frac{9\times 10^9\times 1\times 10^{-6}}{(2\times 10^{-3})^2}\left(\hat{j}\right) + \frac{9\times 10^9\times 1\times 10^{-6}}{(2\times 10^{-3})^2}\left(\hat{i}\right)$$
...
The direction of $\bar{E}_{\mathrm{A}}$ is given by $\frac{\bar{E}_{\mathrm{A}}}{\left|\bar{E}_{\mathrm{A}}\right|} = \frac{2.25\times 10^{9}(\hat{i} +\hat{j})}{2.25\times\sqrt{2}\times 10^{9}} = \frac{(\hat{i} +\hat{j})}{\sqrt{2}}$ which is the unit vector along OA as shown in the figure.

> [Image: Diagram showing direction of field]

The acceleration experienced by an electron placed at point A is

$$\bar{a}_{A} = \frac{\bar{F}}{m} = \frac{q\bar{E}_{A}}{m}$$
$$\qquad = \frac{\left(-1.6\times 10^{-19}\right)\times\left(2.25\times 10^{9}\right)\left(\hat{i} +\hat{j}\right)}{9.1\times 10^{-31}}$$
$$\qquad = -3.95\times 10^{20}\left(\hat{i} +\hat{j}\right)\mathrm{N}\mathrm{kg}^{-1}$$
...
> [Image: Diagram of block on inclined plane with electric field]
...
> [Image: Free body diagram for the mass m]
...
> [Image: Figure 1.9 Electric field lines for isolated positive and negative charges]
...
> [Image: Figure 1.10 Electric field at a point P]
...
> [Image: Figure 1.11 Electric field has larger magnitude at surface A than B]
...
> [Image: Figure 1.12 Two electric field lines never intersect each other]
...
> [Image: Figure 1.13 Electric field lines and magnitude of the charge]
...
> [Image: Figures (a), (b), (c) showing electric field lines]
...
> [Image: Figure 1.14 (a) Electric dipole (b) Electric field lines for the electric dipole]
...
> [Image: Four charge configurations (a), (b), (c), (d)]
...
$$\mathrm{Case~}(\mathbf{d})\bar{p} = -2qa(-\hat{i}) + qa\hat{j} +qa(-\hat{j})$$
$$\qquad = 2qa\hat{i}$$

The water molecule $\mathrm{H}_{2}\mathrm{O}$ has this charge configuration. The water molecule has three atoms (two $\mathrm{H}$ atom and one $\mathrm{O}$ atom). The centres of positive (H) and negative (O) charges of a water molecule lie at different points, hence it possess permanent dipole moment. The electric dipole moment $\bar{p}$ is directed from centre of negative charge to the centre of positive charge, as shown in the figure.
...
> [Image: Figure 1.15 Electric field of the dipole along the axial line]
...
$$\qquad = \frac{1}{4\pi\epsilon_{0}}\frac{q}{\left(r - a\right)^{2}}\hat{p} -\frac{1}{4\pi\epsilon_{0}}\frac{q}{\left(r + a\right)^{2}}\hat{p}$$
...
> [Image: Figure 1.16 Total electric field of the dipole on the axial line]
...
$$\bar{E}_{tot} = \frac{1}{4\pi\epsilon_{0}}\left(\frac{4aq}{r^{3}}\right)\hat{p} (r > > a)$$
$$\mathrm{~s i n c e~}2aq\hat{p} = \bar{p}$$
$$\bar{E}_{tot} = \frac{1}{4\pi\epsilon_{0}}\frac{2\bar{p}}{r^{3}} (r > > a)$$
...
$$\qquad = -\frac{1}{4\pi\epsilon_{0}}\frac{2qa}{\left(r^{2} + a^{2}\right)^{2}}\hat{p}$$
$$\qquad \mathrm{since~}\cos \theta = \frac{a}{\sqrt{r^{2} + a^{2}}}$$
...
> [Image: Figure 1.18 Torque on dipole]
...
The magnitude of the total torque $\tau = \left|\overline{OA}\right|\left|(- q\vec{E})\right|\sin \theta +\left|\overline{OB}\right|\left|q\vec{E}\right|\sin \theta$
...
> [Image: Figure 1.19 The dipole in a non-uniform electric field]
...
> [Image: Figure 1.20 Work done is equal to potential energy]
...
> **Note:** It is easier to compute potential energy difference using potential rather than force. Similarly, solving problems using the concept of potential is simpler than that using the concept of field.
...
> [Image: Figure 1.21 Electrostatic potential at a point P]
...
$$V = \frac{-1}{4\pi\epsilon_0}\int_{\infty}^{r}\frac{q}{r^2}\hat{r}.d\bar{r}$$

The infinitesimal displacement vector, $d\bar{r} = d\bar{r}$ and using $\hat{r},\hat{r} = 1$ we have

$$V = -\frac{1}{4\pi\epsilon_0}\int_{\infty}^{r}\frac{q}{r^2}\hat{r}\cdot d\bar{r} = -\frac{1}{4\pi\epsilon_0}\int_{\infty}^{r}\frac{q}{r^2} dr$$
...
> [Image: Figure 1.24 Electrostatic potential due to collection of charges]
...
> [Image: Diagram with point charge and points P and Q]
...
$$V_{p} = \frac{1}{4\pi\epsilon_{r}}\frac{q}{r_{p}} = \frac{9\times 10^{9}\times 9\times 10^{-6}}{10} = 8.1\times 10^{3}\mathrm{V}$$

Electric potential at point Q is given by

$$V_{Q} = \frac{1}{4\pi\epsilon_{r}}\frac{q}{r_{Q}} = \frac{9\times 10^{9}\times 9\times 10^{-6}}{16} = 5.06\times 10^{3}\mathrm{V}$$
...
> [Image: Diagram showing two charges and point P]

Since the total electric potential at P is zero,

$$V_{tot} = \frac{1}{4\pi\epsilon_{r}}\left(\frac{q}{x} -\frac{2q}{(9 - x)}\right) = 0\mathrm{(or)}$$
...
> [Image: Figure 1.25 Potential due to electric dipole]
...
$$r_{1}^{2} = r^{2}\left(1 - 2a\frac{\cos\theta}{r}\right)$$
$$(\mathrm{or}) r_{1} = r\left(1 - \frac{2a}{r}\cos \theta\right)^{\frac{1}{2}}$$
...
$$\mathrm{since~}\cos (180 - \theta) = -\cos \theta \mathrm{~we~get~}$$
...
$$\frac{1}{r_{2}} = \frac{1}{r}\left(1 + \frac{2a\cos\theta}{r}\right)^{-\frac{1}{2}}$$
...
$$V = \frac{1}{4\pi\epsilon_{0}} q \left[ \frac{1}{r}\left(1 + \frac{a}{r}\cos\theta\right) - \frac{1}{r}\left(1 - \frac{a}{r}\cos\theta\right) \right]$$
$$V = \frac{1}{4\pi\epsilon_{0}} \frac{q}{r} \left( \frac{2a\cos\theta}{r} \right)$$
$$V = \frac{1}{4\pi\epsilon_{0}} \frac{2aq\cos\theta}{r^2}$$
...
$$V = \frac{1}{4\pi\epsilon_{0}} \frac{\vec{p}\cdot \hat{r}}{r^2} \quad (r>>a) \quad (1.38)$$
...
An equipotential surface is a surface on which all the points are at the same electric potential. For a point charge the equipotential surfaces are concentric spherical surfaces as shown in Figure 1.26(b). Each spherical surface is an equipotential surface but the value of the potential is different for different spherical surfaces.

> [Image: Figure 1.26 Equipotential surface of point Charge]
...
> [Image: Figure 1.27 Equipotential surface for uniform electric field]
...
From 1 to 4 cm, the potential is constant, $V = 25V$ . It implies that $\frac{dV}{dx} = 0$ . So $\bar{E} = 0$ From 4 to 5 cm, the slope $\frac{dV}{dx} = - 25V\mathrm{cm}^{- 1}$ . So $\bar{E} = +25V\mathrm{cm}^{- 1} \hat{i}$ .

The plot of electric field for the various points along the $x$ axis is given below.

> [Image: Graph of E vs x]
...
> [Image: Figure 1.28 Electrostatic potential energy for collection of point charges]
...
> [Image: Diagram of square with charges]
...
$$W_{R} = q\left[ \frac{1}{4\pi\epsilon_{0}} \left( \frac{q}{\sqrt{2}a} + \frac{-q}{a} \right) \right] = \frac{1}{4\pi\epsilon_{0}} \frac{q^{2}}{a} \left( \frac{1}{\sqrt{2}} - 1 \right)$$
...
$$W_{S} = -q \left[ \frac{1}{4\pi\epsilon_{0}} \left( \frac{q}{a} + \frac{-q}{\sqrt{2}a} + \frac{q}{a} \right) \right] = \frac{1}{4\pi\epsilon_{0}} \frac{q^{2}}{a} \left( \frac{1}{\sqrt{2}} - 2 \right)$$

Total work done = $W_{P}+W_{Q}+W_{R}+W_{S}$ = $0 + \left(-\frac{q^2}{4\pi\epsilon_0 a}\right) + \frac{q^2}{4\pi\epsilon_0 a}\left(\frac{1}{\sqrt{2}} - 1\right) + \frac{q^2}{4\pi\epsilon_0 a}\left(\frac{1}{\sqrt{2}} - 2\right)$

$$= \frac{q^2}{4\pi\epsilon_0 a} \left( -1 + \frac{1}{\sqrt{2}} - 1 + \frac{1}{\sqrt{2}} - 2 \right) = \frac{q^2}{4\pi\epsilon_0 a} \left( \frac{2}{\sqrt{2}} - 4 \right) = \frac{q^2}{4\pi\epsilon_0 a} \left( \sqrt{2} - 4 \right)$$
...
> [Image: Figure 1.29 The dipole in a uniform electric field]
...
$$|\tau_{ext}| = |\tau_{E}| = pE\sin\theta \quad (1.50)$$

Substituting equation (1.50) in equation (1.49), we get

$$W = \int_{\theta^{\prime}}^{\theta} pE\sin\theta d\theta = pE[-\cos\theta]_{\theta^{\prime}}^{\theta} = -pE\cos\theta + pE\cos\theta^{\prime}$$
...
$$W = \Delta U = U\big(90^{\circ}\big) - U\big(0^{\circ}\big)$$
...
> [Image: Figure 1.30 Electric flux for a point charge]
...
> [Image: Figure 1.31 The electric flux for Uniform electric field]
...
> [Image: Diagram of rectangle in electric field]
...
$$\Phi_{E} = 100\times 5\times 10^{-2}\times 10\times 10^{-2}\times \cos 0^{\circ} = 0.5\mathrm{Nm}^{2}\mathrm{C}^{-1}$$
...
$$\Phi_{E} = \sum_{i=1}^{n} \vec{E}_i \cdot \Delta \vec{A}_i \quad (1.56)$$

By taking the limit $\Delta \vec{A}_i \rightarrow 0$ , the summation becomes integration. Then the electric flux is given by

$$\Phi_{E} = \int \vec{E} \cdot d\vec{A} \quad (1.57)$$

> [Image: Figure 1.32 Electric flux for nonuniform electric Field]
...
> [Image: Figure 1.33 Electric flux over a closed surface]
...
> [Image: Figure 1.34 Total electric flux of point charge]
...
$$\Phi_{E} = \frac{1}{4\pi\epsilon_{0}}\frac{Q}{r^{2}}\times 4\pi r^{2} = 4\pi \frac{1}{4\pi\epsilon_{0}} Q$$

$$\Phi_{E} = \frac{Q}{\epsilon_{0}} \quad (1.61)$$
...
> [Image: Figure 1.35 Gauss law for arbitrarily shaped surface]
...
> [Image: Figure (a) and (b) showing charges and surfaces]
...
Since the magnitude of the electric field for the entire curved surface is constant, $E$ is taken out of the integration and $Q_{e n c l}$ is given by $Q_{e n c l} = \lambda L$ , where $\lambda$ is the linear charge density (charge present per unit length).

$$E\int_{Curved}^{Curved}dA = \frac{\lambda L}{\epsilon_{o}} \quad (1.65)$$

> [Image: Figure 1.37 Cylindrical Gaussian surface]

Here $\int_{\substack{\mathrm{Curved}}}^{\mathrm{Curved}}dA =$ total area of the curved surface $= 2\pi rL$ . Substituting this in equation (1.65), we get
...
> [Image: Figure 1.38 Electric field due to charged infinite planar sheet]
...
$$\Phi_{E} = \oint \bar{E}\cdot d\bar{A}$$
$$= \int_{\substack{\mathrm{Curved}}}^{\mathrm{Curved}}\bar{E}\cdot d\bar{A} +\int_{P}\bar{E}\cdot d\bar{A} +\int_{P^{\prime}}\bar{E}\cdot d\bar{A} = \frac{Q_{e n c l}}{\epsilon_{0}}$$
...
$$\Phi_{E} = \int_{P}EdA + \int_{P^{\prime}}EdA = \frac{Q_{e n c l}}{\epsilon_{0}} \quad (1.69)$$

Since the magnitude of the electric field at these two equal flat surfaces is uniform, E is taken out of the integration and $Q_{e n c l}$ is given by $Q_{e n c l} = \sigma A$ , we get

$$2E \int_{P}dA = \frac{\sigma A}{\epsilon_{0}}$$

The total area of surface either at $\mathrm{P}$ or $\mathrm{P}'$

$$\int_{P}dA = A$$
$$\mathrm{Hence}2EA = \frac{\sigma A}{\epsilon_{0}}\mathrm{or}E = \frac{\sigma}{2\epsilon_{0}}$$
$$\mathrm{In~vector~form},\vec{E} = \frac{\sigma}{2\epsilon_{0}}\hat{n}$$
...
The electric field will be the same at any point farther away from the charged plane. Equation (1.71) implies that if $\sigma >0$ the electric field at any point $\mathrm{P}$ is along outward perpendicular $\hat{n}$ drawn to the plane and if $\sigma < 0$ the electric field points inward perpendicularly to the plane $(- \hat{n})$ .
...
Consider two infinitely large charged plane sheets with equal and opposite charge densities $+\sigma$ and $-\sigma$ which are placed parallel to each other as shown in the Figure 1.39.

> [Image: Figure 1.39 Electric field due to two parallel charged sheets]
...
Consider a point $P$ inside the shell at a distance $r$ from the centre. A Gaussian sphere of radius $r$ is constructed as shown in the Figure 1.40 (b). Applying Gauss law

> [Image: Figure 1.40 The electric field due to a charged spherical shell]

$$\oint_{\mathrm{Gaussian}}E\cdot d\vec{A} = \frac{Q_{encl}}{\epsilon_{\circ}}$$
$$E\cdot 4\pi r^{2} = \frac{Q_{encl}}{\epsilon_{\circ}}$$

Since Gaussian surface encloses no charge, $Q_{encl} = 0$ . The equation (1.77) becomes
...
> [Image: Figure 1.41 Electric field versus distance for a spherical shell of radius $R$]
...
> [Image: Figure 1.42 Electric field of conductors]
...
> [Image: Figure 1.43 No net charge inside the conductor]
...
If the electric field has components parallel to the surface of the conductor, then free electrons on the surface of the conductor would experience acceleration (Figure 1.44(a)). This means that the conductor is not in equilibrium. Hence at equilibrium, the electric field must be perpendicular to the surface of the conductor. This is shown in Figure 1.44(b).

> [Image: Figure 1.44 (a) Electric field is along the surface (b) Electric field is perpendicular to the surface of the conductor]
...
> [Image: Figure 1.45 The electric field on the surface of the conductor]
...
where $\hat{n}$ represents the unit vector outward normal to the surface of the conductor. Suppose $\sigma < 0$ , then electric field points inward perpendicular to the surface. (iv) The electrostatic potential has the same value on the surface and inside of the conductor.
...
> [Image: Figure 1.46 (a) Electric field inside the cavity (b) Faraday cage]
...
> [Image: Figure 1.47 Various steps in electrostatic induction]
...
> [Image: Diagram of projectile motion with electric field]
...
| :--- | :--- | :--- |
| Time of flight T | $2v_0 \sin \theta / g$ | $2v_0 \sin \theta / (g + qE/m)$ |
| Maximum height h_max | $v_0^2 \sin^2 \theta / 2g$ | $v_0^2 \sin^2 \theta / 2(g + qE/m)$ |
| Range R | $v_0^2 \sin 2\theta / g$ | $v_0^2 \sin 2\theta / (g + qE/m)$ |
...
> [Image: Figure 1.48 Non polar molecules (a) without external field (b) with the external field]
...
> [Image: Figure 1.49 (a) Randomly oriented polar molecules (b) Align with the external electric field]
...
$$\vec{P} = \chi_{e} \epsilon_{0} \vec{E}_{ext}$$
...
> [Image: Figure 1.50 Induced electric field lines inside the dielectric]
...
But inside the dielectric, the net charge is zero even in a small volume. So the dielectric in the external field is equivalent to two oppositely charged sheets with the surface charge densities $+ \sigma_{b}$ and $- \sigma_{b}$ . These charges are called bound charges. They are not free to move like free electrons in conductors. This is shown in the Figure 1.50.
...
> [Image: Figure 1.51 (a) Balloon sticks to the wall (b) Polarisation of wall due to the electric field created by the balloon]
...
> [Image: Figure 1.52 (a) Parallel plate capacitor (b) Capacitor connected with a battery (c) Symbolic representation of capacitor.]
...
The SI unit of capacitance is coulomb per volt or farad (F) in honor of Michael Faraday. Farad is a larger unit of capacitance. In practice, capacitors are available in the range of microfaraad $(1\mu \mathrm{F} = 10^{- 6}\mathrm{F})$ to picofaraad $(1\mathrm{pF}$ $= 10^{- 12}\mathrm{F})$ . A capacitor is represented by the symbol $\pm \mathrm{f}$ or $\pm \mathrm{f}$ . Note that the total charge stored in the capacitor is zero $(Q - Q = 0)$ . When we say the capacitor stores charges, it means the amount of charge that can be stored in any one of the plates.
...
> [Image: Figure 1.53 Various types of capacitors]
...
The electric field between two infinite parallel plates is uniform and is given by $E = \frac{\sigma}{\epsilon}$ where $\sigma$ is the surface charge density on either plates $\left(\sigma = \frac{Q}{A}\right)$ . If the separation distance $d$ is very much smaller than the size of the plate $(d^2 < < A)$ , then the above result can be used even for finite- sized parallel plate capacitor.

> [Image: Figure 1.54 Capacitance of a parallel plate capacitor]
...
$$C = \frac{Q}{V} = \frac{Q}{Qd/(A\epsilon_0)} = \frac{\epsilon_0A}{d} \quad (1.84)$$
...
$$C = \frac{\epsilon_0A}{d} = \frac{8.85\times 10^{-12}\times 25\times 10^{-4}}{1\times 10^{-3}}$$
$$\qquad = 221.2\times 10^{-13}\mathrm{F}$$
$$C = 22.12\times 10^{-12}\mathrm{F} = 22.12\mathrm{pF}$$

(b) The charge stored in any one of the plates is $\mathrm{Q} = \mathrm{CV}$ , Then
...
$$U_{E} = \frac{Q^{2}}{2C} = \frac{1}{2} CV^{2}\quad (\therefore Q = CV) \quad (1.87)$$
...
But where is this energy stored in the capacitor? To understand this question, the equation (1.87) is rewritten as follows using the results $C = \frac{\epsilon_0A}{d}$ and $V = Ed$

$$U_{E} = \frac{1}{2}\left(\frac{\epsilon_{0}A}{d}\right)\left(Ed\right)^{2} = \frac{1}{2}\epsilon_{0}\left(Ad\right)E^{2} \quad (1.88)$$

where $Ad =$ volume of the space between the capacitor plates. The energy stored per unit volume of space is defined as energy density $u_{E} = \frac{U}{Volume}$ . From equation (1.88), we get
...
> [Image: Figure 1.55 (a) Flash capacitor in camera (b) Heart defibrillator]
...
> [Image: Figure 1.56 (a) Capacitor is charged with a battery (b) Dielectric is inserted after the battery is disconnected]
...
$$C = \frac{Q_0}{V} = \epsilon_r\frac{Q_0}{V_0} = \epsilon_rC_0 \quad (1.93)$$

Since $\epsilon_{r} > 1$ , we have $C > C_{0}$ . Thus insertion of the dielectric increases the capacitance.

Using equation (1.84),

$$C = \frac{\epsilon_r\epsilon_0A}{d} = \frac{\epsilon A}{d} \quad (1.94)$$
...
Let us now consider what happens when the battery of voltage $V_{0}$ remains connected to the capacitor when the dielectric is inserted into the capacitor. This is shown in Figure1.57.

> [Image: Figure 1.57 (a) Capacitor is charged through a battery (b) Dielectric is inserted when the battery is connected.]
...
$$\mathrm{Now},C_0 = \frac{\epsilon_0A}{d}$$
$$\mathrm{and}C = \frac{\epsilon A}{d}$$
...
| S. No | Condition | Charge Q | Voltage V | Electric field E | Capacitance C | Energy U |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
...
> In a computer keyboard, the key is a part of a capacitor. When the key is pressed, the separation between the plates decreases leading to an increase in the capacitance. This in turn triggers the electronic circuits in the computer to identify which key is pressed.
...
$$Q = CV = 44.25\times 10^{-13}\times 10$$
$$= 442.5\times 10^{-13}C = 44.25\mathrm{pC}$$

The stored energy is

$$U = \frac{1}{2} CV^2 = \frac{1}{2}\times 44.25\times 10^{-13}\times 100$$
$$= 2.21\times 10^{-10}\mathrm{J}$$
...
$$C_0 = \frac{C}{\epsilon_r} = \frac{4.425\times 10^{-12}}{5}$$
$$= 0.885\times 10^{-12}\mathrm{F} = 0.885\mathrm{pF}$$

The stored charge remains same and $44.25\mathrm{pC}$ . Hence newly stored energy is

$$U_{0} = \frac{Q^{2}}{2C_{0}} = \frac{Q^{2}\epsilon_{r}}{2C} = \epsilon_{r}U$$
$$= 5\times 2.21\times 10^{-10}\mathrm{J} = 11.05\times 10^{-10}\mathrm{J}$$
...
> [Image: Figure 1.58 (a) Capacitors connected in series (b) Equivalent capacitors $C_{s}$]
...
$$V = \frac{Q}{C_1} + \frac{Q}{C_2} + \frac{Q}{C_3} = Q\left(\frac{1}{C_1} +\frac{1}{C_2} +\frac{1}{C_3}\right) \quad (1.104)$$
...
> [Image: Figure 1.59 (a) capacitors in parallel (b) equivalent capacitance with the same total charge]
...
> [Image: Figure (a) with capacitors and points P and Q]
...
From the figure (c), we infer that $2\mu \mathrm{F}$ and $4\mu \mathrm{F}$ are connected in parallel. So the equivalent capacitance is given in the figure (d).
...
Consider two conducting spheres A and B of radii $r_{1}$ and $r_{2}$ respectively connected to each other by a thin conducting wire as shown in the Figure 1.60. The distance between the spheres is much greater than the radii of either spheres.

> [Image: Figure 1.60 Two conductors are connected through conducting wire]
...
$$\begin{array}{l}V_{A} = V_{B}\\ \mathrm{or}\frac{q_{1}}{r_{1}} = \frac{q_{2}}{r_{2}} \end{array} \quad (1.111)$$
...
$$\sigma r = \mathrm{constant} \quad (1.113)$$
...
> [Image: Diagram of two spheres connected by wire]
...
The electrostatic potential on the surface of the sphere B is $V_{B} = \frac{1}{4\pi\epsilon_{0}}\frac{q_{2}}{r_{2}}$ Since $V_{A} = V_{B}$ . We have
...
> [Image: Figure 1.61 (a) Charge distribution on an irregular conductor (b) Sharp points have more charge density]

For an irregularly shaped conductor, the surface charge density is not uniform. The charge density is large at the pointed ends (more curved surface) and less at the flatter regions (less curved surface). This is shown in Figure 1.61 (b). The value of the electric field is large at the pointed ends. When a charged conductor has a sharp point, the electric field near that point is very high. This high electric field ionizes the surrounding air. Positive and negative ions are produced in the air. The positive ions are repelled by the positive charge on the conductor and move away from the conductor. Due to this, there is a loss of charge from the conductor. This effect is called action of points or corona discharge.

The working of the lightning arrester is based on this principle.
...
> [Image: Figure 1.62 (a) Schematic diagram of a lightning arrestor. (b) A house with a lightning arrestor]

The lower end of the rod is connected to copper plate which is buried deep into the ground. When a negatively charged cloud is passing above the building, it induces a positive charge on the spike. Since the induced charge density on thin sharp spike is large, it results in a corona discharge. This positive charge ionizes the surrounding air which in turn neutralizes the negative charge in the cloud. The negative charge pushed to the spikes passes through the copper rod and goes to the ground. Thus the building is protected from lightning.
...
> [Image: Figure 1.63 Van de Graaff generator]
...
$$V_{max} = 3\times 10^{6}\times 0.5$$
$$= 1.5\times 10^{6}\mathrm{V}(\mathrm{or})1.5\mathrm{million}\mathrm{volt}$$
...
- Van de Graaff generator is used to produce large potential difference (~107 V).
...
> [Image: Diagram with points A1, A2, B1, B2]

(a) $\mathrm{A}_{1}$ and $\mathrm{A}_{2}$ (b) $\mathrm{B}_{1}$ and $\mathrm{B}_{2}$ (c) both directions
(d) No stable
...
> [Image: Electric field lines pattern]
...
> [Image: Four Gaussian surfaces A, B, C, D with charges]
...
> [Image: Closed surface with charge q inside water]
...
> [Image: Four configurations of point charges]
...
13. A parallel plate capacitor stores a charge $Q$ at a voltage $V$ . Suppose the area of the parallel plate capacitor and the distance between the plates are each doubled then which is the quantity that will change? (a) Capacitance (b) Charge (c) Voltage (d) Energy density
14. Three capacitors are connected in triangle as shown in the figure. The equivalent capacitance between the points A and C is (a) 1μF (b) 2μF (c) 3μF (d) $\frac{1}{4}\mu \mathrm{F}$
15. Two metallic spheres of radii $1\mathrm{cm}$ and $3\mathrm{cm}$ are given charges of $-1\times 10^{-2}\mathrm{C}$ and $5\times 10^{-2}\mathrm{C}$ respectively. If these are connected by a conducting wire, the final charge on the bigger sphere is (AIIPMT -2012) (a) $3\times 10^{-2}\mathrm{C}$ (b) $4\times 10^{-2}\mathrm{C}$ (c) $1\times 10^{-2}\mathrm{C}$ (d) $2\times 10^{-2}\mathrm{C}$
...
**Ans:** $\mathrm{F}_{\mathrm{e}} = 23 \times 10^{23} \mathrm{~N}, \mathrm{~W} = 588 \mathrm{~N}$, $\frac{F_{\mathrm{e}}}{W} = 3.9 \times 10^{21}$
...
> [Image: Three figures for free body diagrams]
...
> [Image: Electron entering electric field]

**Ans:**
$$\bar{a} = -\frac{eE}{m} \hat{j}, \bar{\upsilon} = \upsilon_{o} \hat{i} -\frac{eE}{m} t \hat{j}, \bar{r} = \upsilon_{o} t \hat{i} -\frac{1}{2} \frac{eE}{m} t^{2} \hat{j}$$
...
> [Image: Triangular box in E field]
...
12. An electron and a proton are allowed to fall through the separation between the plates of a parallel plate capacitor of voltage 5 V and separation distance $\mathrm{h} = 1 \mathrm{mm}$ as shown in the figure.

> [Image: Parallel plate capacitor with charges falling]
...
(c) Among the three, which one will reach the bottom first? (Take $\mathrm{m}_{\mathrm{p}} = 1.6 \times 10^{-27} \mathrm{kg}, \mathrm{m}_{\mathrm{e}} = 9.1 \times 10^{-31} \mathrm{kg}$ and $\mathrm{g} = 10 \mathrm{m} \mathrm{s}^{-2}$ )

**Ans:**
$$(a)t_{e} = \sqrt{\frac{2h m_{e}}{e E}}\approx 1.5\mathrm{ns}\mathrm{(ignoring~gravity)},$$
$$t_{p} = \sqrt{\frac{2h m_{p}}{e E}}\approx 63\mathrm{ns}\mathrm{(ignoring~gravity)}$$
$$(b) t_{n} = \sqrt{\frac{2h}{g}}\approx 14.1\mathrm{ms}$$
...
(a) If the bottom part of the cloud is 1000 m above the ground, determine the electric potential difference that exists between the cloud and ground. (b) In a typical lightning phenomenon, around 25 C of electrons are transferred from cloud to ground. How much electrostatic potential energy is transferred to the ground? **Ans:** (a) $V = 3 \times 10^9 \mathrm{~V},$ (b) $U = 75 \times 10^9 \mathrm{~J}$
...
> [Image: Capacitor network]

**Ans:**
$$q_1 = 240 \mu C, q_2 = 240 \mu C, q_3 = 480 \mu C, q_4 = 960 \mu C, q_5 = 480 \mu C$$
$$V_1 = 120 V, V_2 = 60 V, V_3 = 60 V, V_4 = 120 V, V_5 = 120 V$$
$$U_1 = 0.0144 J, U_2 = 0.0072 J, U_3 = 0.0144 J, U_4 = 0.0576 J, U_5 = 0.0288 J$$
...
> [Image: Two capacitor configurations P and Q]

**Ans:** $$C_p = \frac{\epsilon_0 A}{2d}\left(1 + \epsilon_r\right), C_Q = \frac{2\epsilon_0 A}{d}\left(\frac{\epsilon_r}{1 + \epsilon_r}\right)$$
...
> [Image: PhET simulation screenshot]
