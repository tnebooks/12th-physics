...
Let us consider a point object $A$ placed in front of a plane mirror. The point of incidence is $O$ on the mirror. A light ray AO from the point object is incident on the mirror and it is reflected along OB. The normal is ON. The angle of incidence $\angle AON =$ angle of reflection $\angle BON$.
...
| $360/\theta$ | Position of object placed | Number of images $n$ |
|--------------|---------------------------|---------------------|
| Even | Symmetrical | $n = (360/\theta - 1)$ |
| Even | Unsymmetrical | $n = (360/\theta - 1)$ |
| Odd | Symmetrical | $n = (360/\theta - 1)$ |
| Odd | Unsymmetrical | $n = (360/\theta)$ |
...
If the distance between his head $H$ and eye $E$ is $h_1$ and distance between his feet $F$ and eye $E$ is $h_2$. The person's total height is $h$. Here it is $h = h_1 + h_2$.
...
![Relation between $f$ and $R$](image-placeholder)
...
But $PD = AB$ as the aperture of the mirror is small. Therefore,
...
But $PA' = -v$, $PA = -u$, $FA' = -v + f$, $FP = -f$ (sign convention applied). Substituting these in equation (6.8),
...
From the two similar triangles $\Delta ABP$ and $\Delta A'B'P$,
...
**EXAMPLE 6.3**  
An object is placed at a distance of $20.0\ \mathrm{cm}$ from a concave mirror of focal length $15.0\ \mathrm{cm}$.  
(a) What distance from the mirror a screen should be placed to get a sharp image?  
(b) What is the nature of the image?

**Solution**  
Given, $f = -15\ \mathrm{cm}$, $u = -20\ \mathrm{cm}$
...
(b) Magnification, $m = \frac{h'}{h} = -\frac{v}{u}$
...
The angular speed $\omega$ (with unit rad $\mathrm{s^{-1}}$) of the toothed-wheel when the light disappeared for the first time is,
...
In the same time $t$, light travels a distance $d'$ in vacuum with speed $c$. Then,
...
Thus, optical path $d' = n d$.

---

**EXAMPLE 6.5**  
Light travels from air into a glass slab of thickness $50\ \mathrm{cm}$ and refractive index 1.5.  
...
**Solution**  
Given, thickness of glass slab $d = 50\ \mathrm{cm} = 0.5\ \mathrm{m}$, refractive index $n = 1.5$.
...
On the other hand, if light travels from denser to rarer medium, it deviates away from normal. The angle of deviation in this case is $d = r - i$.
...
For the reverse path, the angle of incidence in medium 2 is $r$ and the angle of refraction in medium 1 is $i$. Then,
...
Rewriting for refractive index of oil, $n_o = \frac{n_g}{n_{go}} = \frac{1.5}{1.25} = 1.2$  
...
In triangles $\triangle DOB$ and $\triangle DIB$,
...
$DB$ cancels both sides. Now, $DO$ is the actual depth $d$ and $DI$ is the apparent depth $d'$.

$$
n_1 \frac{1}{d} = n_2 \frac{1}{d'} \quad \Rightarrow \quad \frac{d'}{d} = \frac{n_2}{n_1} \tag{6.25}
$$

Rewriting for apparent depth $d'$,
...
**EXAMPLE 6.7**  
A coin is at the bottom of a trough containing three immiscible liquids of refractive indices 1.3, 1.4 and 1.5 poured one above the other of heights $30\ \mathrm{cm}$, $16\ \mathrm{cm}$, and $20\ \mathrm{cm}$ respectively. What is the apparent depth at which the coin appears to be when seen from air medium outside? In which medium the coin will appear?
...
For example, the refractive index of glass is about 1.5. The critical angle for glass-air interface is $i_c = \sin^{-1}(1/1.5) = 41.8^\circ$. The refractive index of water is 1.33. The critical angle for water-air interface is $i_c = \sin^{-1}(1/1.33) = 48.6^\circ$.
...
**EXAMPLE 6.8**  
What is the radius of the illumination when seen above from inside a swimming pool from a depth of $10\ \mathrm{m}$ on a sunny day? What is the total angle of view? [Given, refractive index of water is $4/3$]
...
The Snell's law in the product form for this refraction at the point $A$ is,

$$
n_3 \sin i_a = n_1 \sin r_a \tag{6.37}
$$

To have the total internal reflection inside the optical fibre, the angle of incidence at the core-cladding interface at $B$ should be at least critical angle $i_c$. Snell's law for the refraction at point $B$ is,
...
Using $\sin r_a = \sqrt{1 - \cos^2 r_a}$ and substituting (6.40) in (6.41),

$$
\sin i_a = \frac{n_1}{n_3} \sqrt{1 - \left(\frac{n_2}{n_1}\right)^2} = \frac{1}{n_3} \sqrt{n_1^2 - n_2^2} \tag{6.42}
$$

If the outer medium is air, then $n_3 = 1$. The acceptance angle $i_a$ is,

$$
i_a = \sin^{-1}\left(\sqrt{n_1^2 - n_2^2}\right) \tag{6.43}
$$

The numerical aperture (NA) of the optical fibre is defined as $\sin i_a$,
...
**Solution**  
Given, $n_1 = 1.5$, $n_2 = 1.48$, $n_3 = 1$ (air).
...
Consider a ray of light $PQ$ incident at an angle $i$ on a parallel-sided glass slab of thickness $t$ and refractive index $n$. The ray refracts at $Q$ with angle $r$ and travels inside the slab. At the other face, it emerges with angle $i$ (by principle of reversibility). The lateral displacement $L$ is the perpendicular distance between the incident ray and the emergent ray.

From geometry, in triangle $QRS$, $QR = \frac{t}{\cos r}$, and in triangle $QRT$, $L = QR \sin(i - r)$.
...
**Solution**  
Given, $t = 0.25\ \mathrm{m}$, $n = 1.5$, $i = 60^\circ$.
...
The primary focus $F_1$ is defined as a point where a point source kept produces a parallel emergent rays to the principal axis after passing through lens. For a converging lens, such an object is a real object and for a diverging lens, it is a virtual object. The distance $PF_1$ is the primary focal length $f_1$.

![Primary focus](image-placeholder)

The secondary focus $F_2$ is defined as a point where all the parallel rays travelling close to the principal axis converge to form an image on the principal axis after passing through lens. For a converging lens, such an image is a real image and for a diverging lens, it is a virtual image. The distance $PF_2$ is the secondary focal length $f_2$.
...
If the lens is kept in air, then $n_1 = 1$ and $n_2 = n$. So,
...
Applying sign convention, $\frac{-h'}{h} = \frac{v}{-u}$. Substituting in (6.65),
...
**EXAMPLE 6.12**  
A biconvex lens has radii of curvature $20\ \mathrm{cm}$ and $15\ \mathrm{cm}$ for the two curved surfaces. The refractive index of the material of the lens is 1.5.  
...
Given, $n = 1.5$, $R_1 = 20\ \mathrm{cm}$, $R_2 = -15\ \mathrm{cm}$.
...
(b) When the lens is flipped, $R_1 = 15\ \mathrm{cm}$, $R_2 = -20\ \mathrm{cm}$. Substituting,
...
Thus $f$ remains the same. Focal length does not change when the lens is flipped.
...
**Solution**  
This lens is called convex-concave lens. Given, $n = 1.52$, $R_1 = 10\ \mathrm{cm}$, $R_2 = 20\ \mathrm{cm}$. Both $R_1$ and $R_2$ are positive.
...
The power of a lens $P$ is the reciprocal of its focal length in meter.
...
For lens ①, object distance $PO = u$, image distance $PI' = v'$,
For lens ②, object distance $PI' = v'$, image distance $PI = v$.
...
The combination acts as a single lens of focal length $f$ so that
...
The total magnification $m$ of the combination is the product of magnifications of individual lenses:
...
**EXAMPLE 6.15**  
What is the focal length of the combination if the lenses of focal lengths $-70\ \mathrm{cm}$ and $150\ \mathrm{cm}$ are in contact? What is the power of the combination?

**Solution**  
$f_1 = -70\ \mathrm{cm}$, $f_2 = 150\ \mathrm{cm}$.
...
Power $P = \frac{1}{f} = \frac{1}{-1.3125\ \mathrm{m}} = -0.76\ \mathrm{D}$.

---

**EXAMPLE 6.16**  
An object of $5\ \mathrm{mm}$ height is placed at a distance of $15\ \mathrm{cm}$ from a convex lens of focal length $10\ \mathrm{cm}$. A second lens of focal length $5\ \mathrm{cm}$ is placed $40\ \mathrm{cm}$ from the first lens and $55\ \mathrm{cm}$ from the object. Find (a) the position of the final image, (b) its nature and (c) its size.

**Solution**  
$h = 5\ \mathrm{mm} = 0.5\ \mathrm{cm}$, $u_1 = -15\ \mathrm{cm}$, $f_1 = 10\ \mathrm{cm}$, $f_2 = 5\ \mathrm{cm}$, distance between lenses $d = 40\ \mathrm{cm}$.
...
Magnification $m_1 = \frac{v_1}{u_1} = \frac{30}{-15} = -2$, so $h' = m_1 h = -2 \times 0.5 = -1\ \mathrm{cm}$ (inverted, real).

This image acts as object for second lens. The distance between lenses is $40\ \mathrm{cm}$, so object distance for second lens $u_2 = 40 - 30 = 10\ \mathrm{cm}$. Since the image from first lens is to the right of the first lens, and the second lens is to the right, the object for second lens is real and on the left? Actually careful: first lens forms image at $30\ \mathrm{cm}$ to its right. Second lens is placed $40\ \mathrm{cm}$ from the first lens, so the image is $10\ \mathrm{cm}$ to the left of the second lens. Thus $u_2 = -10\ \mathrm{cm}$.
...
Magnification $m_2 = \frac{v_2}{u_2} = \frac{10}{-10} = -1$.

Total magnification $m = m_1 \times m_2 = (-2) \times (-1) = 2$.

Final image height $h'' = m \times h = 2 \times 0.5 = 1\ \mathrm{cm}$ (positive, erect).

Final image is formed $10\ \mathrm{cm}$ to the right of the second lens, real? Since $v_2$ positive, image is real. But magnification positive indicates erect, which is contradictory for real image? Wait, the second lens forms image at $10\ \mathrm{cm}$ to its right. The object for second lens was real ($u_2 = -10\ \mathrm{cm}$) and $f_2 = 5\ \mathrm{cm}$ positive, so $v_2 = 10\ \mathrm{cm}$ positive gives a real image. The magnification $m_2 = v_2/u_2 = 10/(-10) = -1$ gives inverted image. So total magnification = (-2)*(-1)=2, so final image is erect and magnified? Actually $m_1 = -2$ means first image inverted and magnified. $m_2 = -1$ means second image inverted relative to the first, so final image is erect (since two inversions give erect). So final image is erect, real? Real images are usually inverted, but here we have an erect real image? That is possible if the image is formed on the same side as the object? Actually with a converging lens, a real image is inverted. But here the second lens forms a real image (since $v_2$ positive) but the magnification is negative, so it's inverted. So final image should be inverted overall? Let's recalc: first image inverted (real). Second lens: object is real (on left) and we get real image on right. For a converging lens, real object gives real image inverted. So second image is inverted relative to the first. So final image is inverted relative to original object? Actually first image inverted means it's upside down. Second image inverted relative to that means it's upright relative to original. So final image is upright. But can a real image be upright? With a single lens, a real image is always inverted. However, with two lenses, the final image can be real and upright if the intermediate image is inverted and then the second lens inverts it again. That is possible. So the final image is real and erect.

Thus, (a) final image is $10\ \mathrm{cm}$ to the right of the second lens, (b) real and erect, (c) size $1\ \mathrm{cm}$.
...
**EXAMPLE 6.17**  
A thin biconvex lens is made up of a glass of refractive index 1.5. The two surfaces have equal radii of curvature of $30\ \mathrm{cm}$ each. One of its surfaces is made reflecting by silvering it from outside.  
...
**Solution**  
Given $n = 1.5$, $R_1 = 30\ \mathrm{cm}$, $R_2 = -30\ \mathrm{cm}$ (assuming the silvered surface is the second surface).

First find $f_l$ using lens maker's formula:
...
The angle of deviation $d_1$ at surface AB is $d_1 = i_1 - r_1$.  
The angle of deviation $d_2$ at surface AC is $d_2 = i_2 - r_2$.  
Total angle of deviation $d = d_1 + d_2 = (i_1 + i_2) - (r_1 + r_2)$.

In the quadrilateral AQNR, two angles are right angles, so $\angle A + \angle QNR = 180^\circ$. In triangle QNR, $r_1 + r_2 + \angle QNR = 180^\circ$. Comparing, we get $r_1 + r_2 = A$.
...
The angle of deviation depends on the angle of incidence $i_1$, the angle of emergence $i_2$, and the angle of the prism $A$. But for a given angle of incidence, the angle of emergence is determined by the refractive index of the material of the prism.

---

**EXAMPLE 6.18**  
A monochromatic light is incident on an equilateral prism at an angle $30^\circ$ and is emergent at an angle of $75^\circ$. What is the angle of deviation produced by the prism?

**Solution**  
Since the prism is equilateral, $A = 60^\circ$. Given $i_1 = 30^\circ$, $i_2 = 75^\circ$.
...
![Graph between $i$ and $d$](image-placeholder)

The minimum value of angle of deviation is called angle of minimum deviation $D$. At minimum deviation,
(i) the angle of incidence is equal to the angle of emergence, $i_1 = i_2 = i$.
(ii) the angle of refraction at the face one and face two are equal, $r_1 = r_2 = r$.
...
Each colour in vacuum has a definite wavelength. Red has the longest wavelength of about $700\ \mathrm{nm}$ while violet has the shortest wavelength of about $400\ \mathrm{nm}$. All colours travel with the same speed in vacuum. In a medium, the red colour travels with the highest speed and violet colour with the least speed. Hence, the refractive index of the material of the prism is different for different colours. For violet, the refractive index is the highest and for red it is the least.
...
Let $\delta_v$ and $\delta_R$ be the angles of minimum deviation for violet and red colour, and $n_v$ and $n_R$ the corresponding refractive indices. Then

$$
\delta_v = (n_v - 1)A, \quad \delta_R = (n_R - 1)A
$$

The angular dispersion between the two extreme colours is $\delta_v - \delta_R = (n_v - n_R)A$.

If $\delta$ is the angle of minimum deviation for any mean colour (green or yellow) and $n$ the corresponding refractive index, then $\delta = (n - 1)A$.

Dispersive power $\omega$ is defined as the ratio of the angular dispersion to the deviation for the mean colour:
...
If the scattering of light is by atoms and molecules which have size much less than the wavelength $\lambda$ of light ($a \ll \lambda$), then the scattering is called Rayleigh scattering. The intensity of light scattered in Rayleigh scattering is inversely proportional to the fourth power of wavelength:
...
- The relation between focal length and radius of curvature in spherical mirror is $2f = R$ (or) $f = \frac{R}{2}$.
...
- The magnification in spherical mirror is $m = \frac{h'}{h} = -\frac{v}{u}$, $m = \frac{f - v}{f} = \frac{f}{f - u}$.
...
- Law of refraction (Snell's law) in ratio form: $\frac{\sin i}{\sin r} = \frac{n_2}{n_1}$; in product form: $n_1 \sin i = n_2 \sin r$.
- The relative refractive index of second medium with respect to first medium is $n_{21} = \frac{n_2}{n_1}$.
- The apparent depth is always lesser than actual depth: $d' = \frac{d}{n}$.
- Critical angle $i_c$ is the angle of incidence for which the angle of refraction is $90^\circ$. For $i > i_c$, total internal reflection occurs. $\sin i_c = \frac{1}{n}$.
- Snell's window radius: $R = \frac{d}{\sqrt{n^2 - 1}}$.
- Optical fibre makes use of total internal reflection. Acceptance angle: $i_a = \sin^{-1}\left(\sqrt{n_1^2 - n_2^2}\right)$; Numerical aperture: $\mathrm{NA} = \sqrt{n_1^2 - n_2^2}$.
- Lateral displacement in glass slab: $L = t \frac{\sin(i - r)}{\cos r}$.
...
- Magnification by lens: $m = \frac{v}{u} = \frac{f}{f+u} = \frac{f-v}{f}$.
- Power of a lens: $P = \frac{1}{f}$ (in meters). Unit: dioptre (D).
- Effective focal length of lenses in contact: $\frac{1}{f} = \frac{1}{f_1} + \frac{1}{f_2}$.
- Angle of deviation in prism: $d = i_1 + i_2 - A$.
- At minimum deviation: $i_1 = i_2 = i$, $r_1 = r_2 = r = A/2$, and $n = \frac{\sin\left(\frac{A+D}{2}\right)}{\sin\left(\frac{A}{2}\right)}$.
- Dispersive power: $\omega = \frac{n_v - n_R}{n - 1}$.
- Rayleigh scattering: $I \propto 1/\lambda^4$.
...
2. A rod of length $10\ \mathrm{cm}$ lies along the principal axis of a concave mirror of focal length $10\ \mathrm{cm}$ in such a way that its end closer to the pole is $20\ \mathrm{cm}$ away from the mirror. The length of the image is (AIPMT Main 2012)  
   (a) $2.5\ \mathrm{cm}$ (b) $5\ \mathrm{cm}$ (c) $10\ \mathrm{cm}$ (d) $15\ \mathrm{cm}$
...
3. An object is placed in front of a convex mirror of focal length $f$ and the maximum and minimum distance of an object from the mirror such that the image formed is real and magnified. (JEE Main 2009)  
   (a) $2f$ and $c$ (b) $c$ and $\infty$ (c) $f$ and $0$ (d) None of these
...
5. If the velocity and wavelength of light in air is $V_a$ and $\lambda_a$ and that in water is $V_w$ and $\lambda_w$, then the refractive index of water is,  
   (a) $\frac{V_w}{V_a}$ (b) $\frac{V_a}{V_w}$ (c) $\frac{\lambda_w}{\lambda_a}$ (d) $\frac{V_a \lambda_a}{V_w \lambda_w}$
...
8. The radius of curvature of curved surface at a thin planoconvex lens is $10\ \mathrm{cm}$ and the refractive index is 1.5. If the plane surface is silvered, then the focal length will be,  
   (a) $5\ \mathrm{cm}$ (b) $10\ \mathrm{cm}$ (c) $15\ \mathrm{cm}$ (d) $20\ \mathrm{cm}$

9. An air bubble in glass slab of refractive index 1.5 (near normal incidence) is $5\ \mathrm{cm}$ deep when viewed from one surface and $3\ \mathrm{cm}$ deep when viewed from the opposite face. The thickness of the slab is,  
   (a) $8\ \mathrm{cm}$ (b) $10\ \mathrm{cm}$ (c) $12\ \mathrm{cm}$ (d) $16\ \mathrm{cm}$
...
1. An object of $4\ \mathrm{cm}$ height is placed at $6\ \mathrm{cm}$ in front of a concave mirror of radius of curvature $24\ \mathrm{cm}$. Find the position, height, magnification and nature of the image.  
   [Ans: $v = 12\ \mathrm{cm}$, $h' = 8\ \mathrm{cm}$, $m = 2$, image is erect, virtual, twice the height of object formed on right side of mirror.]
...
- Here $n_1$ = refractive index of air ($n_1 = 1$). $n_2$ is the refractive index of the lens. $R_1$ and $R_2$ are the radius of curvature of the lens.  
- Take $n_2 = 1.5$, $R_1 = 3\ \mathrm{cm}$ and $R_2 = -3\ \mathrm{cm}$. Calculate the focal length of the lens using the formula $\frac{1}{f} = (n_2 - 1)\left(\frac{1}{R_1} - \frac{1}{R_2}\right)$. Verify your answer with the value obtained on the screen by adjusting $n_2$, $R_1$ and $R_2$.  
- Find out the curvatures $R_1$ and $R_2$ to make a lens of desired focal length $7\ \mathrm{cm}$.  
- When $R_1 = -1\ \mathrm{cm}$ and $R_2 = 1\ \mathrm{cm}$, you will get a concave lens of focal length $-1\ \mathrm{cm}$. Try to construct concave and convex lenses by adjusting $R_1$ and $R_2$.  
- Adjust the refractive index $n_2$ and analyse how focal length changes with respect to refractive index.  
- Adjust the refractive index $n_1$ and analyse how focal length changes with respect to refractive index of the medium surrounded by the lens.
