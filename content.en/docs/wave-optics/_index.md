...
For the two right angle triangles $\triangle ABB'$ and $\triangle AA'B'$

$$
\frac{\sin i}{\sin r} = \frac{BB'/AB'}{AA'/AB'} = \frac{BB'}{AA'} = \frac{v_1}{v_2} = \frac{v_1}{v_2} \times \frac{c}{c} = \frac{c/v_2}{c/v_1}
$$

Here, $c$ is speed of light in vacuum. The ratio $c/v$ is a constant, called refractive index of the medium. The refractive index of medium (1) is $c/v_1 = n_1$ and that of medium (2) is $c/v_2 = n_2$.
...
The speed of light is inversely proportional to the refractive index of the medium $v \propto 1/n$ and also directly proportional to wavelength of light $v \propto \lambda$. Hence,
...
**EXAMPLE 7.1**  
The wavelength of light from sodium source in vacuum is $5893\ \text{Å}$. What are its (a) wavelength, (b) speed and (c) frequency when this light travels in water which has a refractive index of 1.33.

**Solution**  
The refractive index of vacuum, $n_1 = 1$  
The wavelength in vacuum, $\lambda_1 = 5893\ \text{Å}$  
The speed in vacuum, $c = v_1 = 3\times 10^8\ \mathrm{m\ s^{-1}}$  
The refractive index of water, $n_2 = 1.33$

The wavelength of light in water, $\lambda_2$  
...
Rewriting, $\lambda_2 = \frac{n_1}{n_2} \times \lambda_1$
...
Rewriting, $v_2 = \frac{n_1}{n_2} \times v_1$
...
$\beta \propto \lambda$ and wavelength decreases in a medium: $\lambda' = \frac{\lambda}{RI}$  
Thus $\frac{\beta'}{\beta} = \frac{\lambda'}{\lambda} = \frac{1}{RI}$ so $\beta' = \frac{\beta}{RI} = \frac{6\times 10^{-3}}{4/3} = 4.5\times 10^{-3}\ \mathrm{m} = 4.5\ \mathrm{mm}$.
...
For anti-reflection, the reflected rays should interfere destructively. For minimum thickness, $2\mu d = \lambda/2$ (since $n=1$ in equation (7.36) gives $2\mu d = \lambda$? Wait careful: From (7.36), destructive reflection occurs when $2\mu d = n\lambda$. For $n=1$, $2\mu d = \lambda$, so $d = \frac{\lambda}{2\mu} = \frac{589\times 10^{-9}}{2\times 1.25} = 235.6\ \mathrm{nm}$.
...
The same way the slit is divided into six equal parts to explain the third minimum. The condition for P to be third minimum is $\frac{a}{6}\sin \theta = \frac{\lambda}{2}$,

$$
a \sin \theta = 3\lambda \tag{7.39}
$$

**Condition for $P$ to be $n^{\text{th}}$ minimum**  
Dividing the slit into $2n$ number of (even number of) equal parts makes the light produced by one of the corresponding points to be cancelled by its counterpart. Thus, the condition for $n^{\text{th}}$ minimum is,
...
For points of maxima, the slit is to be divided into odd number of equal parts so that one part remains un-cancelled making the point $P$ appear bright.
...
The central maximum is called $0^{\text{th}}$ order maximum. The points of the maximum intensity lie nearly midway between the successive minima.
...
(a) Equation for diffraction minimum is $a \sin \theta = n\lambda$. The central maximum is spread up to the first minimum. Hence $n=1$.

$\sin \theta = \frac{\lambda}{a} = \frac{500\times 10^{-9}}{0.2\times 10^{-3}} = 2.5\times 10^{-3}$, so $\theta \approx 2.5\times 10^{-3}\ \mathrm{rad}$ (since small angle).

Angular spread of central maximum is $2\theta = 5.0\times 10^{-3}\ \mathrm{rad}$.

(b) Distance from central maximum to first minimum: $y_1 = \frac{\lambda D}{a} = \frac{500\times 10^{-9} \times 60\times 10^{-2}}{0.2\times 10^{-3}} = 1.5\times 10^{-3}\ \mathrm{m} = 1.5\ \mathrm{mm}$.

For second minimum ($n=2$): $a \sin \theta = 2\lambda$, so $y_2 = \frac{2\lambda D}{a} = 2\times 1.5\ \mathrm{mm} = 3.0\ \mathrm{mm}$.

The distance between central maximum and second minimum is $y_2 - y_1 = 3.0 - 1.5 = 1.5\ \mathrm{mm}$.

---

**EXAMPLE 7.10**  
A monochromatic light of wavelength $5000\ \text{Å}$ passes through a single slit producing diffraction pattern for the central maximum as shown in the figure. Determine the width of the slit.

**Solution**  
From the figure (not provided), the first minimum occurs at an angle such that the path difference is $\lambda$. The width $a = \frac{\lambda}{\sin \theta}$. With given data (not fully provided in text), we would compute.
...
(i) If $a < \lambda$, then $\sin \theta > 1$ which is not possible. Hence, diffraction does not take place.  
(ii) If $a = \lambda$, then $\sin \theta = 1$ i.e. $\theta = 90^\circ$. The first minimum is at $90^\circ$. Hence, the central maximum spreads fully into the geometrically shadowed region leading to the bending of the diffracted light by $90^\circ$.  
...
Equating the above two equations for $\theta$ gives $\frac{\lambda}{a} = \frac{a}{2z}$.
...
**Solution**  
$a = 0.5\ \mathrm{mm} = 5\times 10^{-4}\ \mathrm{m}$, $\lambda = 500\ \mathrm{nm} = 500\times 10^{-9}\ \mathrm{m}$.
...
Condition for $P$ to be zero$^{\text{th}}$ maximum, $m = 0$: $(a+b)\sin \theta = 0$ thus $\sin \theta = 0$, its position $\theta = 0$. This is called zero$^{\text{th}}$ diffraction (or) central maximum. It is formed at an angle $0$.

Condition for $P$ to be first maximum, $m = 1$: $(a+b)\sin \theta_1 = \lambda$. The first maximum is obtained at an angle $\theta_1$.

Condition for $P$ to be second maximum, $m = 2$: $(a+b)\sin \theta_2 = 2\lambda$.

Condition for $P$ to be $m^{\text{th}}$ maximum: On either side of central maximum, different higher order diffraction maxima are formed at different angular positions.
...
Note: In a single slit experiment, the formula $a\sin \theta = n\lambda$ is condition for minimum with $n$ as order of minimum. But in the grating experiment, the formula $\sin \theta = Nm\lambda$ is condition for maximum with $m$ as the order of diffraction.
...
**Solution**  
Number of lines $= 4000\ \mathrm{cm}^{-1}$, $m = 2$, $\theta = 30^\circ$.

Number of lines per unit length: $N = \frac{4000}{1\times 10^{-2}} = 4\times 10^5\ \mathrm{m}^{-1}$.

Equation for diffraction maximum for grating is $\sin \theta = Nm\lambda$.

Thus $\lambda = \frac{\sin \theta}{Nm} = \frac{0.5}{4\times 10^5 \times 2} = \frac{1}{16\times 10^5} = 6.25\times 10^{-7}\ \mathrm{m} = 6250\ \text{Å}$.

---

**EXAMPLE 7.13**  
A monochromatic light of wavelength $500\ \mathrm{nm}$ strikes a grating and produces fourth order maximum at an angle of $30^\circ$. Find the number of slits per centimeter.

**Solution**  
$\lambda = 500\ \mathrm{nm} = 500\times 10^{-9}\ \mathrm{m}$, $m = 4$, $\theta = 30^\circ$.

$\sin \theta = Nm\lambda \Rightarrow N = \frac{\sin \theta}{m\lambda} = \frac{0.5}{4\times 500\times 10^{-9}} = 2.5\times 10^5\ \mathrm{m}^{-1} = 2.5\times 10^3\ \mathrm{cm}^{-1}$.
...
From the geometry, $\theta = \frac{r_0}{f}$. Substituting for $\theta$ in equation (7.53) and rearranging gives
...
**Solution**  
$a = 2.3\ \mathrm{m}$, $\lambda = 589\ \mathrm{nm} = 589\times 10^{-9}\ \mathrm{m}$.

Angular resolution $\theta = \frac{1.22\lambda}{a} = \frac{1.22\times 589\times 10^{-9}}{2.3} = 3.124\times 10^{-7}\ \mathrm{rad}$ (about $0.0011'$).
...
**Solution**  
As the intensity of the unpolarised light falling on the first polaroid is $I$, the intensity of polarized light emerging from it will be $I_0 = \frac{I}{2}$. Let $I'$ be the intensity of light emerging from the second polaroid.

Malus' law: $I' = I_0 \cos^2 \theta = \frac{I}{2} \cos^2 30^\circ = \frac{I}{2} \left(\frac{\sqrt{3}}{2}\right)^2 = \frac{I}{2} \cdot \frac{3}{4} = \frac{3I}{8}$.
...
**Solution**  
(a) After first polaroid, intensity $I_0 = I/2$. The second polaroid is at $90^\circ$, so $I' = I_0 \cos^2 90^\circ = 0$. No light comes out.

(b) Let first polaroid be $P_1$, second $P_2$, third $P_3$ inserted between them at $45^\circ$. Angle between $P_1$ and $P_3$ is $45^\circ$. Intensity after $P_3$: $I_1 = I_0 \cos^2 45^\circ = \frac{I}{2} \cdot \frac{1}{2} = \frac{I}{4}$. Then light passes through $P_2$ with angle $45^\circ$ between $P_3$ and $P_2$: $I_2 = I_1 \cos^2 45^\circ = \frac{I}{4} \cdot \frac{1}{2} = \frac{I}{8}$.
...
For glass: $\tan i_p = 1.5$, $i_p = \tan^{-1}(1.5) = 56.3^\circ$.  
For water: $\tan i_p = 1.33$, $i_p = \tan^{-1}(1.33) = 53.1^\circ$.
...
**Solution**  
$n = 1.65$, $\tan i_p = 1.65$, $i_p = \tan^{-1}(1.65) = 58.8^\circ$.  
The inclination with the horizontal surface is $90^\circ - i_p = 31.2^\circ$.
...
- If $I_1$ and $I_2$ are the intensities of interfering light, then $I_{\max} = I_1 + I_2 + 2\sqrt{I_1I_2}$, $I_{\min} = I_1 + I_2 - 2\sqrt{I_1I_2}$.
- If interfering lights have equal intensities $I_0$ with phase difference $\phi$, then $I = 4I_0 \cos^2(\phi/2)$.
...
- In Young's double slit experiment, position of $n^{\text{th}}$ bright fringe: $y_n = n\frac{\lambda D}{d}$.
- Position of $n^{\text{th}}$ dark fringe: $y_n = \frac{(2n-1)\lambda D}{2d}$.
- Bandwidth: $\beta = \frac{\lambda D}{d}$.
...
- In thin films, for reflected rays: constructive $2\mu d = (2n-1)\frac{\lambda}{2}$, destructive $2\mu d = n\lambda$.
- For transmitted rays: constructive $2\mu d = n\lambda$, destructive $2\mu d = (2n-1)\frac{\lambda}{2}$.
...
- Single slit diffraction: $n^{\text{th}}$ minimum: $a\sin\theta = n\lambda$; $n^{\text{th}}$ maximum: $a\sin\theta = (2n+1)\frac{\lambda}{2}$.
- Fresnel's distance: $z = \frac{a^2}{2\lambda}$.
- Diffraction grating: $(a+b)\sin\theta = m\lambda$, $\sin\theta = Nm\lambda$.
- Rayleigh's criterion: angular resolution $\theta = \frac{1.22\lambda}{a}$, spatial resolution $r_0 = \frac{1.22\lambda f}{a}$.
...
- Brewster's law: $\tan i_p = n$.
- Simple microscope: near point focusing $m = 1 + \frac{D}{f}$, normal focusing $m = \frac{D}{f}$.
- Compound microscope: near point $m = \left(\frac{L}{f_o}\right)\left(1 + \frac{D}{f_e}\right)$, normal $m = \left(\frac{L}{f_o}\right)\left(\frac{D}{f_e}\right)$.
- Astronomical telescope: $m = \frac{f_o}{f_e}$, $L = f_o + f_e$.
- Resolving power of microscope: $R_M = \frac{2n\sin\beta}{1.22\lambda}$.
- Myopia correction: concave lens of focal length $f = -x$.
- Hypermetropia correction: convex lens $f = \frac{y \times 25}{y - 25}$.
...
5. When light is incident on a soap film of thickness $5\times 10^{-5}\ \mathrm{cm}$, the wavelength of light reflected maximum in the visible region is $5320\ \text{Å}$. Refractive index of the film will be,  
...
6. First diffraction minimum due to a single slit of width $1.0\times 10^{-5}\ \mathrm{cm}$ is at $30^\circ$. Then wavelength of light used is,  
   (a) $400\ \text{Å}$ (b) $500\ \text{Å}$ (c) $600\ \text{Å}$ (d) $700\ \text{Å}$
...
7. Discuss the diffraction at single slit and obtain the condition for $n^{\text{th}}$ minimum.  
8. Discuss the diffraction at a grating and obtain the condition for the $m^{\text{th}}$ maximum.  
...
2. In Young's double slit experiment, 62 fringes are seen on a screen for sodium light of wavelength $5893\ \text{Å}$. If violet light of wavelength $4359\ \text{Å}$ is used in place of sodium light, how many fringes will be seen?  
   [Ans: 84]
...
5. Light of wavelength $5000\ \text{Å}$ produces diffraction pattern of the single slit of width $2.5\ \mu\mathrm{m}$. What is the maximum order of diffraction possible?  
   [Ans: 5]

6. $I_0$ is the intensity of light existing between two cross Polaroids kept with their axes perpendicular to each other. A third polaroid is introduced between them. What must be the angle between the axes of first and the newly introduced polaroid to get the maximum light from the whole arrangement?  
   [Ans: $45^\circ$]
