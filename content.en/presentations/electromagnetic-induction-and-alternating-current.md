---
title: "Electromagnetic Induction and Alternating Current"
layout: "single"
categories: ["12th Physics"]
tags: ["EMI", "AC", "Faraday Law", "Transformer", "Tamil Nadu Board"]
summary: "Covers electromagnetic induction, Faraday’s laws, Lenz’s law, alternating current, and transformers."
---

# Electromagnetic Induction and Alternating Current
### Class 12 – Tamil Nadu State Board
---

## Core Concept
This chapter explains how **electric current can be generated using magnetic fields** and the behavior of **alternating current (AC)**.

* Basis of electric generators
* Foundation for power transmission
* Widely used in electrical systems

---

## Electromagnetic Induction
Production of induced emf when magnetic flux changes.

* Discovered by **:contentReference[oaicite:0]{index=0}**
* Occurs when:
  - Magnet moves near coil
  - Coil moves in magnetic field
  - Magnetic field changes with time

---

## Magnetic Flux
Measure of magnetic field passing through a surface.

:contentReference[oaicite:1]{index=1}

* Unit: Weber (Wb)

---

## Faraday’s Laws of Induction

* First Law: Change in magnetic flux induces emf  
* Second Law: Magnitude of emf depends on rate of change of flux  

:contentReference[oaicite:2]{index=2}

---

## Lenz’s Law
Direction of induced current opposes the cause.

* Ensures conservation of energy
* Represented by negative sign in Faraday’s law

---

## Self and Mutual Induction

* **Self Induction:** emf induced in same coil  
* **Mutual Induction:** emf induced in nearby coil  

:contentReference[oaicite:3]{index=3}

---

## Alternating Current (AC)

* Current that changes direction periodically  
* Produced by AC generators  

:contentReference[oaicite:4]{index=4}

---

## RMS Values

* Effective value of AC current and voltage  

:contentReference[oaicite:5]{index=5}

---

## AC Circuits

### Pure Resistor
* Current and voltage in phase  

### Pure Inductor
* Current lags voltage  

### Pure Capacitor
* Current leads voltage  

---

## Transformer

* Device used to change voltage level  
* Works on mutual induction  

:contentReference[oaicite:6]{index=6}

---

## Applications
Importance in daily life:

* Electric generators  
* Transformers in power distribution  
* Household electricity (AC supply)  
* Motors and appliances  

---

## Interactive Practices
1. **Numericals:** emf and flux calculations  
2. **Concept Questions:** Lenz’s law applications  
3. **Circuit Analysis:** AC circuits behavior  

---

## Conclusion
> "Electromagnetic induction is the backbone of power generation."

**Next Step:** Practice derivations and AC circuit problems for exams.